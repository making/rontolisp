#!/usr/bin/env bash
# Recompile solids.lisp to a browser-loadable WebAssembly reactor.
# --no-wasi drops all WASI imports, so the module's only imports are the host
# functions solids.lisp declares with rontolisp:wasm-import ("gl" and "canvas");
# --optimize tree-shakes the runtime and the spliced geom/linalg libraries so
# only the reachable functions ship.
set -euo pipefail
...
echo "compiling solids.lisp -> solids.wasm"
java -jar "$jar" "$here/solids.lisp" -o "$here/solids.wasm" --no-wasi --optimize
...
echo "then open http://localhost:8000/webgl-solids/"
