...
超えるとクラスがロードできなくなり**（`.todo/17`）、隠れ層 20・重み 1.25 万個という
「4 バックエンドすべてで動く上限」がモデルの上限そのものになっていました。

いまは重みを **`weights.bin`（RLW1 バイナリ）から起動時に読み込みます**。定数として焼き込まないので
この上限は効かず、パラメータは 15 万個（604KB）に増えました。`.todo/17` のコンパイラ側の課題
...
java -jar $JAR infer.lisp -o /tmp/infer-c.wasm --component && \
  wasmtime run -W gc=y -W component-model-more-async-builtins=y --dir . /tmp/infer-c.wasm < samples/u.txt
```
