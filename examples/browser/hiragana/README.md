...
`examples/browser/wasm-browser/` がコンパイル済み `.wasm` をブラウザで動かす最小例なのに対し、
こちらはそこに「機械学習」と「canvas 入力」を載せたものです。WASI Preview1 シムは
`examples/browser/wasm-browser/wasi-shim.js` と同一のものをコピーして使っています。
...
- ネットワークは **576 - 20 - 46**。出力 46 ユニット + one-hot ターゲット + argmax で多クラス分類
  （`examples/ml/mlp.lisp` の sigmoid + 二乗誤差バックプロップをそのまま多出力化）。
...
examples/browser/hiragana/gen.sh             # 学習 (数分) して infer.wasm を生成
```
...
```bash
examples/browser/hiragana/regen-glyphs.sh   # glyphgen/GlyphGen.java を実行して字形を再生成
```
...
```bash
python3 examples/browser/hiragana/tools/k49/train_k49.py                 # 実データ学習 -> weights-k49.lisp
examples/browser/hiragana/gen.sh --weights-from examples/browser/hiragana/weights-k49.lisp  # 焼き込み -> infer.wasm
```
...
S=examples/browser/hiragana/samples

# インタプリタ
java -jar $JAR examples/browser/hiragana/infer.lisp < $S/u.txt        # -> pred 2 u

# JVM (クラス名は出力ファイル名になるのでパスを含めない)
cd examples/browser/hiragana && java -jar ../../$JAR infer.lisp -o Infer.class && java Infer < samples/u.txt

# WASM Preview1
java -jar $JAR examples/browser/hiragana/infer.lisp -o /tmp/infer.wasm && wasmtime run -W gc /tmp/infer.wasm < $S/u.txt
```
