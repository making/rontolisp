...
(defun augment-glyph (base)
  ...
  (let ((out (list base (dilate base) (dilate (dilate base))
                   (shift-image base -1 0) (shift-image base 1 0)
                   (shift-image base 0 -1) (shift-image base 0 1))))
    (dotimes (i *variants-per-glyph*)
      (let* ((img (warp-image base (rnd-range -0.25 0.25) (rnd-range -0.25 0.25)))
...
(defun k49-load (path limit)
  ...
  (with-open-file (s path :element-type '(unsigned-byte 8))
    (unless (and (= (read-byte s) 72) (= (read-byte s) 75)
                 (= (read-byte s) 66) (= (read-byte s) 49))
      (error "not an HKB1 file (run tools/k49/prepare-k49.py first)"))
    (let* ((count (%read-be32 s))
           (grid (read-byte s))
           (pixels (* grid grid))
           (n (min limit count))
           (x (make-array (list n 1 grid grid) :element-type 'double-float
                          :initial-element 0.0))
  ...
  (let* ((n (length samples))
    ...
    (dolist (sample samples)
      (let ((img (car sample))
            (base (* i *pixels*)))
        (dotimes (k *pixels*)
          (setf (row-major-aref x (+ base k)) (aref img k)))
...
(defun concat-batches (a b)
  ...
  (let* ((xa (first a)) (la (second a))
         (xb (first b)) (lb (second b))
         ...
         (x (make-array (list n 1 *grid* *grid*) :element-type 'double-float
                        :initial-element 0.0))
         (lab (make-array n :element-type 'double-float :initial-element 0.0)))
    (dotimes (k (linalg:size xa)) (setf (row-major-aref x k) (row-major-aref xa k)))
...
(defun shuffle-batch (b)
  ...
  (let ((p (linalg:permutation (car (linalg:shape (first b))))))
    (list (linalg:take-rows (first b) p)
          (linalg:take-rows (second b) p))))
