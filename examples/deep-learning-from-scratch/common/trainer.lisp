...
(defun train (net params x-train t-train x-test t-test
              &key (epochs 3) (mini-batch-size 16) optimizer
              (verbose t) (eval-limit nil))
      ...
      (let* ((batch-mask (linalg:choice train-size mini-batch-size))
        ...
        (when (= (mod (+ i 1) iter-per-epoch) 0)
          ...
          (let ((train-count (net-accuracy-count net x-train-eval t-train-eval))
            ...
            (setq acc-list
                  (cons (list train-count test-count) acc-list))
            (when verbose
              (format t "=== epoch ~a: train acc ~a/~a, test acc ~a/~a ===~%"
                      epoch train-count n-train-eval
                      test-count n-test-eval))))))
    (reverse acc-list)))
