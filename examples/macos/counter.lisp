...
;;;; which is what `java:` interop cannot do -- compiled to a JVM class or jar,
;;;; which carries the binding inside it, and as a --native executable on Apple
;;;; silicon, whose runner is the binding; never as a .wasm. It needs a display, so
;;;; it is not in examples.yaml.
;;;;
...
;;;;   ./target/rontolisp examples/macos/counter.lisp --native -o counter && ./counter
;;;;
