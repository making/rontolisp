...
`./build.sh && npx wrangler dev`, then `npx wrangler deploy` — with one
exception, `hello-ningle/`, whose module builds but does not instantiate yet for
a reason the table below names.
...
| [`hello-ningle/`](hello-ningle) | **`hello-clack` with [ningle](https://github.com/fukamachi/ningle)** — the third shape: the application is a CLOS *object* you hang routes on, a bare string is a controller, and the 404 is an overridden `ningle:not-found` **method**. **Does not instantiate on a Worker yet** — ningle reads requests through `lack-request`, whose chain draws `random` at load time (the trap [`httpbin-clack/`](httpbin-clack/README.md#middleware-lackbuilder-sessions) measures); everything below `build.sh` works, and `check.lisp` drives it locally on every backend. | 2.7 MB (**608 KB gzip**), `--no-wasi` wasm-GC, zero imports | `hello-clack/src/index.js`, byte-identical |
...
`hello-ningle/` to compare the two routing models rather than to deploy — it is
the one directory here that does not run on Cloudflare yet, and the table says
why. Everything except the last step works: `check.lisp` drives the whole
application on the interpreter, the JVM and wasmtime, which is where the
comparison with `hello-tiny-routes/` actually happens.
