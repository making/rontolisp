...
| [`httpbin-clack-one-source/`](httpbin-clack-one-source) | **No `worker.lisp` at all**: `build.sh` compiles [`net/httpbin-clack.lisp`](../net/httpbin-clack.lisp) unchanged, the file that binds a socket locally, deploys as a Servlet war and serves under `wasmtime serve`. One source, five hosts | GENERATED — the same file `httpbin-clack` gets |
