...
| [`hello-ningle/`](hello-ningle) | **`hello-clack` with [ningle](https://github.com/fukamachi/ningle)** — the third shape: the application is a CLOS *object* you hang routes on, a bare string is a controller, and the 404 is an overridden `ningle:not-found` **method**. The heaviest of the three: ningle reads every request through `lack-request`, and that chain is ~2 MB of the module. | 2.7 MB (**608 KB gzip**), `--no-wasi` wasm-GC, zero imports | `hello-clack/src/index.js`, byte-identical |
