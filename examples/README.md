...
| [`gpu-matmul.lisp`](ml/gpu-matmul.lisp) | The same product at **single-float** width — the width a Mac's GPU can take. One `linalg:matmul`, run flagless, with `--simd` and with `--gpu --simd`; the entries are integers below 8, so the printed trace is exact and only the elapsed time moves. See the [GPU guide](../doc/en/guides/gpu-acceleration.md) |
