...
[`scene-solids.lisp`](macos/scene-solids.lisp) and
[`scene-robot-arm.lisp`](macos/scene-robot-arm.lisp) are what a modelled machine looks like
when the camera and the frame loop are already written.
...
| [`scene-solids.lisp`](macos/scene-solids.lisp) | Every [`geom`](../doc/en/guides/solid-modeling.md) primitive on a shelf, in a window: `scene:viewer` opens it, `scene:add` fills it and `scene:fit` frames it -- three lines from a model to a picture, with nothing required and nothing copied, since `geom`, `metal` and `scene` all ship inside the interpreter. `scene:axes :both` draws each solid's own frame beside the world's, and `geom:volume` is printed to the terminal beside each shape as the gauge |
