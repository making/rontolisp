...
| [`httpbin-clack.lisp`](net/httpbin-clack.lisp) | The **Clack** flavour of `httpbin.lisp`: the same five echo endpoints served through `clack:clackup` on the real clack (`ql:quickload`, the built-in `:server :rontolisp` handler backend) instead of the `rontolisp:http-handler` directive. Since rontolisp's server protocol *is* Clack's, only the edges change -- the application is a plain function value rather than a literally quoted defun name, and clack's `:raw-body` is a **synchronous** stream (`nil` when there is no body), so the body is drained with `read-char` and the handler is an ordinary `defun` instead of an `async-defun` awaiting `read-all`. This ONE file is also, unchanged, the deployed Cloudflare Worker of [`cloudflare-workers/httpbin-clack/`](cloudflare-workers/httpbin-clack): `:server :rontolisp` picks the target's transport at compile time, and `--no-wasi` selects the reactor shape with the synthesized `handle-request` export. See the [Clack guide](../doc/en/guides/clack.md) |
| [`httpbin-tiny-routes.lisp`](net/httpbin-tiny-routes.lisp) | The **routed** flavour of `httpbin-clack.lisp`: the same five echo endpoints with the hand-written `cond` over `:path-info` -- and the method check with it -- replaced by [tiny-routes](https://github.com/jeko2000/tiny-routes) (`ql:quickload "tiny-routes/lite"`). Each endpoint is declared with the macro for the one method it answers, so a wrong method **declines** and reaches the single catch-all, which is where the 405 and the 404 are told apart; `/status/:code` adds the path template a `cond` cannot spell. Everything below the quickload is [`cloudflare-workers/httpbin-tiny-routes/worker.lisp`](cloudflare-workers/httpbin-tiny-routes/worker.lisp) verbatim down to `*app*` |
| [`httpbin-ningle.lisp`](net/httpbin-ningle.lisp) | The **ningle** flavour of `httpbin-clack.lisp`, and the other routing model: where `httpbin-tiny-routes.lisp` composes a list of route handlers, [ningle](https://github.com/fukamachi/ningle) (`ql:quickload "ningle"`) hangs routes on an application **object** -- `(make-instance 'ningle:app)` and one `setf` per endpoint, for the one method it answers. A controller receives the matched **parameters** rather than the environment (the environment is `lack.request:request-env` on the `ningle:*request*` special, so `httpbin-clack.lisp`'s helpers are unchanged), and the 405/404 split is answered by overriding the **method** `ningle:not-found` instead of by a catch-all route. `/status/:code` is the same path template a `cond` cannot spell |
...
| [`cloudflare-workers/`](cloudflare-workers) | Running on **Cloudflare Workers** (`npx wrangler deploy`), as independent Worker projects that answer different questions. [`hello/`](cloudflare-workers/hello) is the floor: three `rontolisp:wasm-export`ed functions a Worker calls like JavaScript ones, `--no-gc`, **563 bytes**, zero imports, no shim and no allocator code. [`hello-clack/`](cloudflare-workers/hello-clack) is the floor for the OTHER kind of Worker: a real Clack application in three forms -- `ql:quickload`, one `defun`, `(clack:clackup #'app :server :reactor ...)` -- with no Worker-specific code in the Lisp at all, because the `:reactor` handler backend stores the app and the compiler synthesizes the exported entry point from a marker its `run` leaves behind. [`httpbin/`](cloudflare-workers/httpbin) is the five echo endpoints reached through one exported `handle-request` (JSON string in, JSON string out) instead of `rontolisp:http-handler`, which is what brings the `__ronto_alloc_mark`/`_reset` arena bracket that keeps a resident instance's linear memory flat (measured over 44 000 requests); its application half is [`net/httpbin-clack.lisp`](net/httpbin-clack.lisp) VERBATIM -- a portable Clack application -- and the thirty lines under it are the `clackup` adapter WRITTEN OUT, so that clack itself never enters the module; `--no-wasi` still leaves it importing *nothing*, and `handler-case` lets it return `"json": null` for an unparseable body the way [`net/httpbin.lisp`](net/httpbin.lisp) could not. [`httpbin-clack/`](cloudflare-workers/httpbin-clack) answers the portability question the other two cannot, and it contains NO Lisp file at all: its `build.sh` compiles [`net/httpbin-clack.lisp`](net/httpbin-clack.lisp) ITSELF -- one source whose unchanged `(clack:clackup #'app :server :rontolisp ...)` serves a socket on the interpreter and the JVM, serves under `wasmtime serve` as a component, and under `--no-wasi` takes the handler backend's REACTOR shape, where its `run` stores the app and leaves the compiler a marker to synthesize the exported entry point from (the same shared `rontolisp::%http-reactor-*` machinery the explicit `:server :reactor` backend rides), so running the upstream file curls the identical application on a real server for comparison with `wrangler dev`; and the price is the whole of clack and lack in the module (103 KB gzip, 2.1x `httpbin/`) -- the two directories run the SAME application text on the SAME `src/index.js`, so measuring them against each other isolates exactly what clack costs, which is module size and isolate startup and not per-request time. [`httpbin-component/`](cloudflare-workers/httpbin-component) builds `httpbin/`'s `worker.lisp` as a REACTOR component (`--component --no-wasi` -- it imports nothing, and its top-level forms run at instantiation) and transpiles it with `jco`: the canonical ABI does delete all the memory code, at the price of 95 KB of generated glue and three non-obvious jco flags. Settled along the way, all verified under `wrangler dev`: workerd runs wasm-GC *and* wasm exception handling with no flags, and forbids runtime WebAssembly compilation -- which is what rules out jco's default output |
