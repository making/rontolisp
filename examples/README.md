...
| [`cloudflare-workers/`](cloudflare-workers) | Running on **Cloudflare Workers** (`npx wrangler deploy`), as four independent Worker projects that answer four different questions. [`hello/`](cloudflare-workers/hello) is the floor: three `rontolisp:wasm-export`ed functions a Worker calls like JavaScript ones, `--no-gc`, **563 bytes**, zero imports, no shim and no allocator code. [`httpbin/`](cloudflare-workers/httpbin) is the Cloudflare port of [`net/httpbin.lisp`](net/httpbin.lisp) -- the same five echo endpoints, but reached through one exported `handle-request` (JSON string in, JSON string out) instead of `rontolisp:http-handler`, which is what brings the `__ronto_alloc_mark`/`_reset` arena bracket that keeps a resident instance's linear memory flat (measured over 20 000 requests); `--no-wasi` still leaves the module importing *nothing*, and `handler-case` lets it return `"json": null` for an unparseable body the way the original could not. [`httpbin-clack/`](cloudflare-workers/httpbin-clack) answers the portability question the other two cannot: its handler is a real **Clack application**, and it lives in its own file -- `app.lisp` is [`net/httpbin-clack.lisp`](net/httpbin-clack.lisp) with the `clackup` line removed and nothing added, so the environment plist in / Clack response list out means the same function also runs on hunchentoot, on woo and under `wasmtime serve`; `clackup` is the only thing the Worker replaces, in a separate four-form `worker.lisp` that writes no adapter at all -- the bridge is `clack-handler-cloudflare-workers`, a built-in Clack handler backend (the sibling of `clack-handler-rontolisp`) whose `handle` rides the same `%http-make-env` / `%http-normalize-response` entry points every rontolisp transport meets in, and a third file `serve.lisp` puts `clackup` back so the SAME application can be curl'd on a real server and compared with `wrangler dev`; and the price is the whole of clack and lack in the module (334 KB gzip, 3.7x `httpbin/`). [`httpbin-component/`](cloudflare-workers/httpbin-component) builds that same `app.lisp` as a component and transpiles it with `jco`: the canonical ABI does delete all the memory code, at the price of 247 KB of generated glue, three hand-written WASI stubs, three non-obvious jco flags, and no way to run top-level forms. Settled along the way, all verified under `wrangler dev`: workerd runs wasm-GC *and* wasm exception handling with no flags, and forbids runtime WebAssembly compilation -- which is what rules out jco's default output |
