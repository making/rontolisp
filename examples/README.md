...
| [`kv-server.lisp`](kv-server.lisp) | In-memory key-value store on port 7777 with a line protocol (`SET`/`GET`/`DEL`/`COUNT`/`QUIT`): hash-table state that survives across connections, command parsing with `char=`/`subseq`, and `cond` dispatch. Drive it with `nc` or pipe commands through `echo-client.lisp` (same port). Same WASM notes as `echo-server.lisp` |
