...
| [`asdf/`](asdf) | Loading REAL third-party libraries through `asdf:load-system`: unmodified [split-sequence v2.0.1](https://github.com/sharplispers/split-sequence) and [parse-number v1.8](https://github.com/sharplispers/parse-number) sources, exercised on all four backends (interpreter / JVM / WASM Preview 1 / `--component`); the directory README has the per-backend run commands and expected output |
