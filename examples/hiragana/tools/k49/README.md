...
```bash
# 1) 実データで学習し、weights-k49.lisp を生成（examples/hiragana/ 直下に出力）
python3 examples/hiragana/tools/k49/train_k49.py

# 2) その重みを焼き込んで infer.wasm を生成（gen.sh の後半だけが走る）
examples/hiragana/gen.sh --weights-from examples/hiragana/weights-k49.lisp
