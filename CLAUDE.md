...
- **`eval` is supported in the interpreter and WASM, but not the JVM compiler**:
  ...
  - *WASM*: a small interpreter (`_eval`) is emitted into the module (`WasmRuntimeBuilder.buildEvalBody`) and invoked by `WasmEvalCompiler`. It only appears when the program actually calls `eval` (`WasmLispCompiler.programUsesEval`); otherwise trivial stubs keep the fixed function indices stable. Dispatch is enabled for every registered arity when eval is used. The key trick: because the `StringTable` deduplicates strings, a quoted symbol (e.g. `'car`) compiles to the same data offset as the registry entry for `"car"`, so operator resolution is a plain `i32` offset comparison rather than byte-wise string compare. A compile-time registry blob (records of `nameOffset, funcId, arity`) is appended after the string data and scanned by `_lookup`. `_eval` supports self-evaluating atoms, `quote`/`if`/`progn`, variadic `+ - * /` (left-folded via the binary wrapper) and `list`, and application of any registered function (built-in wrappers + user defuns). It does not support free-variable lookup, `let`, or `lambda`. Built-ins other than the special-cased ones are applied with their first-class wrapper arity.
  - *JVM*: unsupported (standalone `.class` output has no evaluator).
