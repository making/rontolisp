...
- **Dependency shim systems** (`eval/ShimLibraries` + `BuiltinSystems`): `closer-mop` (no slot metaobjects -- class introspection answers nil), `flexi-streams` (a flexi stream IS the underlying stream), `float-features` (over `%ieee754-*`), `trivial-gray-streams`, `uiop` (package stub) -- the usocket rationale: per-implementation portability layers that cannot support rontolisp from their side. Line + real-library replacement plan: `.todo/147`. This makes `(ql:quickload '#:com.inuoe.jzon)` (the real jzon v1.1.4) work end-to-end on the interpreter (`JzonE2eTest`).
