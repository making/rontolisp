...
- **CLOS static subset** (`defclass`/`defgeneric`/`defmethod`/`make-instance`/`slot-value`): the defstruct pattern extended -- a `ClosRegistry` + ONE generated dispatcher defun per generic (single dispatch on arg 1). Single inheritance; no qualifiers/`call-next-method`, no MOP, no `--no-gc`. `.kb/clos.md`, `.todo/040`
