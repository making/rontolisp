...
- **Macro-time setf-place replay (`macro-time-setf-places.txt`)**: on the compile path a macro that reads a global at macro-EXPANSION time (e.g. cl-who's `with-html-output` reading `*html-mode*`) cannot see a runtime `(setf (place) ...)`, so `UserMacroExpander` re-evaluates a top-level `(setf (PLACE) VALUE)` into its macro-time evaluator when PLACE's unqualified member name is listed in `src/main/resources/am/ik/rontolisp/eval/macro-time-setf-places.txt`. **Add a line to that file (NOT a Java branch) when a loadable library exposes a `(defun (setf CONFIG) ...)` place that its own compile-time-expanded macros read** -- e.g. cl-who's `html-mode`. Requirements: the place must be a pure configuration setter (the form is replayed at compile time, so external side effects would double-run), and add a `resource-config.json` entry only if a NEW library type triggers the load (the existing `UserMacroExpander` entry already covers the file). This is a WORKAROUND for the absence of dynamic (special) variable binding; removal/generalization is tracked in `.todo/82`. Details: `.kb/asdf.md` (cl-who paragraph).
