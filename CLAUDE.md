...
- **Lambda lists** (`&optional`/`&rest`/`&key`/`&aux`/`&allow-other-keys`): `LambdaLists` desugars everything to the one native shape "required + `&rest`". No `&whole`, no runtime-`eval` lambdas, no `--no-gc`; `&environment` is legal in MACRO lambda lists only (stripped, bound to nil). `.kb/lambda-lists.md`
- **`do`/`return`/`%block` + named `block`/`return-from`**: `return` is a non-local exit to the nearest enclosing plain boundary (`%block`/`(block nil ...)`, named blocks skipped), and works mid-expression on every backend (the JVM discards the abandoned operands). INTERPRETER: `block`/`return-from` are REAL named exits (BlockReturnSignal caught by the matching block; defun bodies = a block named after the function, defmethod bodies = one named after the generic; `%block` is transparent to the named signal, so a return-from crosses loops and closures within the dynamic extent). COMPILE PATH (JVM + wasm-GC): LEXICAL named exits — `block` is a name-keyed goto/br target, defun/lambda bodies containing return-from are wrapped in the `%fn-block` function boundary (defun name / nil), and a `return-from` targets the matching block in the SAME function, falling back to `%fn-block` (so a return-from inside a lambda stays lambda-local — the deviation from CL); wasm return-from inlines escaped unwind-protect cleanups like `go`. `--no-gc` keeps the name-dropping lowering, no return-from. `.kb/do-return-block.md`
- **`tagbody`/`go` + `prog`/`prog*`**: all three backends. Interpreter = dynamic `go` (`GoSignal` exception + label-indexed re-entry in `evalTagbody`, crosses function boundaries); the compilers support the LEXICAL subset only -- a `go` must target a lexically enclosing tagbody in the same function (JVM = `JvmTagbodyCompiler` goto/patch with every label a `joinShape` join point at the entry stack shape + `JvmGoCompiler` escaped-cleanup/spill unwind mirroring `return`; WASM = `WasmTagbodyCompiler` dispatch loop + `br_table` over segment blocks with an i31-boxed pc, go inlines escaped unwind-protect cleanups, rejects `await` inside). `prog` = `%block` + `let` + `tagbody`, so a user `(return x)` exits the prog.
- **jzon-driven CL additions (compile path largely closed 2026-07-18)**: ALL-BACKEND now: `shiftf`, `load-time-value` (lite: re-evaluates), `(setf (values ...))`, setf through a `(the T place)` wrapper, `typep` (literal type specifiers via the shared static type-test builder), `subtypep` (built-in lattice + class registry, single value; shared `LispMacroExpander.subtypep`, compilers FOLD literal specifiers at compile time via `expandSubtypep`), `(symbolp nil)`/`(symbolp t)` = t, `|...|` reader symbol escape (verbatim, whitespace included), `equalp` compares arrays element-wise (prelude), `streamp`/`'stream` type tests accept the `t` designator (`*standard-output*`'s value), `#'format` is a first-class function (interpreter = rebuild-literal-call; compilers = a `REFERENCE_GATED_FUNCTIONS` wrapper over the runtime control renderer, injected only when `#'format` is referenced), CLOS `list`/`cons`/`sequence` specializers EXCLUDE class instances (tag-union guard; `standard-object`-or-cons-shaped TYPE specializers also trigger dispatcher regeneration on defclass), defstruct options (`:constructor`/`:conc-name`/`:predicate`/`:copier`), the lite introspection stubs (`mask-field`/`scale-float`/`char-name` (prelude defun)/`fdefinition`/`file-position`/`file-length`/`make-broadcast-stream`/`pathnamep`/`input-stream-p`/`output-stream-p`/`stream-element-type`/`class-of`/`slot-boundp`/`slot-makunbound`/`simple-condition-format-control`/`-arguments`, write-string `:start`/`:end`, replace nil bounds -- compilers = shared `LispMacroExpander` expansions; slot-boundp/slot-makunbound need a literal slot name there). `%ieee754-*` = interpreter + JVM (`JvmIeee754Compiler`); WASM lacks the 64-bit unsigned model. ALSO ALL-BACKEND (2026-07-18): fill-pointered/adjustable STRINGS -- interpreter = a mutable `LispString`; compilers = the general array marked "character vector" (JVM length-4 header + `_strv` normalizer, WASM meta-offset-1 marker + always-emitted `_charvec_to_str`), `replace`/`(setf (schar ...))` mutate it in place via the shared runtime-`%arrayp`-branch expansions, character `:initial-contents` copies to a fresh string. `.kb/adjustable-arrays.md`. ALSO ALL-BACKEND (2026-07-18, the jzon full-library sweep): runtime (non-literal) `subtypep` (one shared `%subtypep-runtime` dispatch defun injected per program by `expandTopLevelDefinitions` -- inlining it per call site overflowed the JVM 16-bit branch range), a runtime SYMBOL datum to `error` WITH initargs re-dispatches as a condition type on the compilers too (registry `member` cond; keyword literals stay unbound in the temps, datum-only calls keep the object path -- `.kb/error-handling.md`), runtime slot names for `slot-value`/`slot-boundp` + compiled `%class-slot-defs` (`.kb/clos.md`), `open`/`with-open-file` accept the CL keyword shape (`:external-format`/`:if-exists`/`:if-does-not-exist` only with the native default values), `#'aref` wrapper (variadic row-major fold), `*standard-output*`/`*error-output*` read as the designator `t` on the compilers, `(lambda ())` returns nil, `array-dimension-limit` is reader-substituted like `most-positive-fixnum`, non-literal `symbol-function` and `uiop:` stub calls compile into cold-path runtime signals, `(aref a)` reads a rank-0 array, and WASM `hash-table-p` no longer matches general arrays (both share TYPE_CELL; the header car tells them apart). Pinned by `runtime-type-dispatch-residue` + `JzonE2eTest` (4 backends). Backend-free everywhere: `case`/`ecase` keys match both the package-qualified and plain spelling.
- **Gray streams (ALL backends)**: rontolisp OWNS the protocol (`eval/GrayStreamsLibrary`, `gray.lisp`: `rontolisp:fundamental-character-output-stream` + `rontolisp:stream-write-char`/`-string` generics); `write-string`/`write-char` handed a CLOS instance dispatch to it (interpreter = the builtin wrapper; a defclass extending a Gray base class eager-loads gray.lisp). Compile path = `GrayStreamsLibrary.process` pre-pass (cli + playground + `AsdfLibraryE2eSupport`): splices gray.lisp unless a load already did and rewrites write-string/write-char call sites onto the `rontolisp::%gray-write-*-dispatch` defuns. `trivial-gray-streams` is a built-in shim system ADAPTING onto that protocol -- the core knows no third-party name. `.kb/gray-streams.md`
- **Dependency shim systems** (`eval/ShimLibraries` + `BuiltinSystems`): `closer-mop` (`class-slots` = `(name declared-type)` pairs from `%class-slot-defs` over the ClosRegistry -- slot `:type` is recorded; a "slot metaobject" is just that pair), `flexi-streams` (a flexi stream IS the underlying stream), `float-features` (over `%ieee754-*`), `trivial-gray-streams`, `uiop` (package stub) -- the usocket rationale: per-implementation portability layers that cannot support rontolisp from their side. Line + real-library replacement plan: `.todo/147`. `ShimLibraries` also substitutes LEAF MODULES -- individual component files inside a real system -- consulted by both system loaders: jzon's numeric leaves (`eisel-lemire`/`ratio-to-double`/`schubfach`) become shims over native float arithmetic (float text = rontolisp's shape, not shortest-round-trip; the `#.` table crash and u64/u128 arithmetic vanish with the files). This makes `(ql:quickload '#:com.inuoe.jzon)` (the real jzon v1.1.4) work end-to-end on ALL FOUR backends (`JzonE2eTest`).
...
- **`defstruct`**: a shared `LispMacroExpander` expansion into plain defuns over a tagged list; top-level only on the compile path. Options `:constructor` (incl. lite BOA: lambda-list-named slots read the parameter, the rest evaluate initforms)/`:conc-name`/`:predicate`/`:copier` + a dropped docstring; struct names work as `defmethod` specializers (tag test via `ClosRegistry.registerStruct`). No `:include`/`#S`, no `--no-gc`. `.kb/defstruct.md`
- **CLOS static subset** (`defclass`/`defgeneric`/`defmethod`/`make-instance`/`slot-value`): the defstruct pattern extended -- a `ClosRegistry` + ONE generated dispatcher defun per generic. Specializers on ANY required parameter (methods ordered by leftmost-first per-parameter rank); lambda lists may continue past the required params (`&optional`/`&rest` -- the dispatcher forwards a `&rest` tail via `apply`); `defgeneric` accepts inline `(:method ...)` clauses; `defclass` accepts `:default-initargs`; slot `:reader`/`:accessor` names are METHODS of a generic (same name over different positions in unrelated classes works, and merges with user defmethods; writes go through a `%setf-` writer generic); `initialize-instance :after` runs on `make-instance` (an identity default primary is synthesized). Single inheritance; `:before`/`:after`/`:around` + `call-next-method` (required-args-only forwarding for variadic generics); no MOP, no `--no-gc`. `with-slots` is a symbol-macro-style substitution (writes go through to slots) PLUS a let-bound fallback copy for macro-generated references. `.kb/clos.md`
- **`flet`/`labels`**: expansions to let-bound lambdas plus a Lisp-2 body rewrite. `macrolet` works (interpreter + `UserMacroExpander`); `symbol-macrolet` unsupported. `.kb/flet-labels.md`
- **Dynamic (special) variables**: `defvar`/`declaim special` + `let` = a **shallow binding** (save/set/restore over the global cell), thread-scoped on the interpreter. Local `(declare (special x))` is honored pessimistically (program-wide); special-named lambda/macro parameters bind dynamically on the interpreter. A special-named `let` on the compilers DUAL-BINDS (dynamic set + a captured lexical for closures; reads dynamic-first, setq writes both) and `return`/`return-from` exits restore crossed bindings. Compile-path lite limits: `progv` = compile error, a WASM `return` through an unwind-protect trampoline does not restore, `--no-gc` rejects specials. `.kb/dynamic-special-variables.md`
...
- **Packages**: `cl`/`cl-user`/`rontolisp` resolved by one `PackageResolver` pass with CL colon semantics; nicknames + `:import-from` + `:shadow` (a shadowed name resolves package-locally, before the cl branch); `:shadowing-import-from` rejected. `.kb/packages.md`
- **Reader features** (`#+`/`#-`, `*features*`, `#|...|#`, `#.`): resolved entirely in the frontend against `reader.Features`, so a compiled program's feature set is fixed at compile time; the emitted runtime reader knows none of it. EXCEPTIONS on the interpreter: `*features*` is a real global variable (NOT substituted -- substitution corrupted binding positions), and a loaded file's `#.` datum is wrapped in a `(%read-eval datum)` marker resolved against the global env just before its top-level form evaluates (a marker split by a backquote template resolves through the `%read-eval` identity function instead); reader labels `#n=`/`#n#` work everywhere (non-circular only). Compile path: `#.` works too -- marker read in `RontoLispCli`/`LoadInliner`, resolved per top-level form against the macro-time evaluator in `UserMacroExpander`, plus a `%read-eval` identity emit in both compilers; the playground Compile buttons keep the read error. `.kb/reader-features.md`
...
- **ASDF subset** (`asdf:defsystem`/`load-system`, `ql:quickload`): an API-compatible mini-ASDF, NOT a port -- `.asd` files are parsed as plain data and spliced inside the `LoadInliner` recursion. Unsupported clauses are hard errors. `.kb/asdf.md`, `.todo/054`
...
- **`read`/`load`/`read-line`/file + string streams**: a runtime reader is emitted into compiled output; a stream is an opaque backend-local integer handle; `print`-family and `format` take stream arguments on all three backends. On `--component`, an ASYNC program's stdin reads (`read-line`/`read-char`/`read-byte`) migrate onto `stdin.lisp` over `wasi:cli/stdin@0.3.0` bound FROM the fixed import block (spliced by `eval/StdinLibrary`, promoted like socket reads, no new flags); a NON-async stdin program stays byte-identical on the preview1 adapter branch. `.kb/read-load-streams.md`
- **`rontolisp:fetch` (outgoing HTTP)**: interpreter/JVM use `HttpClient`; WASM is component-only. `.kb/fetch-http.md`
- **`rontolisp:http-handler` (incoming HTTP)**: handler takes a request plist and returns a response plist (headers included, both directions); interpreter (JDK `HttpServer`), JVM (generated class implements `HttpHandlerSupport.Handler`, needs the jar on the classpath), WASM `--component` (`wasi:http/handler@0.3.0`, a CALLBACK async lift -- everything base component-model-async, no gated wasmtime feature -- whose response is delivered mid-task via `canon task.return`; run under `wasmtime serve -W gc=y -W exceptions=y`, wasmtime 46+; wasmCloud wash 2.5.2 hosts it via `wash dev`). A handler that awaits must be an `rontolisp:async-defun`. fetch + serve are ONE Lisp library (`http.lisp` over wit-imported `wasi:http@0.3.0`, spliced by `eval/HttpLibrary` following the reachable half). Preview-1 WASM = compile error. Spin cannot run it (no wasm-GC); wasmCloud hosts it (released wash, `dev.wasm_proposals: [gc, exception-handling, component-model-async]`). `.kb/fetch-http.md`
- **`rontolisp:tcp-*` / `tls-*` + the `usocket` shim**: socket handles live in the file-stream handle space, so the stream ops work on them. TLS is interpreter/JVM only; WASM tcp is component-only -- `sockets.lisp` over a wit-imported `wasi:sockets/types@0.3.0` (spliced by `eval/SocketsLibrary`; no hand-written adapter, no dedicated blob variant), so tcp composes with fetch AND serve in one component, the address accessors are real there, and a tcp component is asyncMode+EH (`wasmtime -W exceptions=y -S tcp=y -S inherit-network=y`); sync surface = `%future-force`, async bodies get the `WasmSocketsRewrite` await promotion. `.kb/tcp-sockets.md`
