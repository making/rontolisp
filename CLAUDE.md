...
- **Compile-time `load` include (`LoadInliner`, cli pkg)**: on the **compile path only** (`RontoLispCli.compileToFile`, before the compilers' `PackageResolver` pass), a **top-level** `(load "literal.lisp")` is spliced in place with the loaded file's forms (recursively, with a path-stack cycle guard), so Pass 1 sees the loaded `defun`s and compiles them natively — no `--dynamic`, no perf loss. Only a literal-path, top-level `load` qualifies; a computed or nested `load` is left untouched (runs at runtime via the embedded reader). Not idempotent (each `load` includes again, matching CL). **File-relative paths**: a relative `load` resolves against the directory of the loading file (the entry file at the top level), falling back to CWD for the top-level entry / REPL — applied to BOTH the inliner and the runtime `load` via `SourceLoader.resolve`/`parentDir` (a `null`/empty base dir skips all `java.nio` math, so the no-FS browser loader is untouched). Interpreter threads a `loadDirStack` (`LispEvaluator.setLoadBaseDir`, seeded by `RontoLispCli` with the entry dir, pushed per nested `load`); the inliner takes a `baseDir` arg. So `java -jar JAR examples/hiragana/infer.lisp` resolves its `(load "common.lisp")` from the repo root. The **interpreter keeps its runtime `load`** (no inliner), so there is no double-definition. This is what lets a program split across files (a thin driver that `(load "lib.lisp")`s shared `defun`s) compile on JVM/WASM. Tests: `LoadInlinerTest`.
...
- **`java:` interop (interpreter + JVM-compiler Java reflection bridge)**: a fourth built-in package `java` (`LispNames.JAVA_PKG`, registered in `PackageRegistry`; does NOT use `cl`) with five functions — `java:new`/`java:call`/`java:static`/`java:field`/`java:proxy`. Interpreter: wrapped host objects are a `LispVal` permittee `LispJavaObject(Object ref)` (prints `#<java <class>>`); the bridge is `eval/JavaInterop.java`, wired in `LispEvaluator.registerJava()` (registered alongside eval/load because `java:proxy` applies a user callback via the evaluator's `apply`). **JVM compiler**: supported via an embedded bridge — `codegen.jvm.JavaBridgeTemplate` is a self-contained rewrite of `JavaInterop` against the compiled value representation (a wrapped host object is the raw reference; `"t"` = true; an ArrayList with a leading `Long` cols header = vector; identical costs — KEEP THE TWO IN SYNC), whose project-compiled bytecode is read from the classpath by `JvmJavaRuntimeBuilder`, renamed into the default package as `RontoLispJavaBridge` (constant-pool Utf8 rewrite, `renameClass`), base64-embedded, and `Lookup.defineClass`'d at first use by the emitted `_javaInit` (guard field `_javaInited`; call sites in `JvmJavaInteropCompiler`; the template must have NO nested classes/records — lambdas are fine — and NO rontolisp imports), so the output stays a single `.class` (but needs a JRE ≥ the build JRE). `usesJava` forces `usesEval` (`java:proxy`/auto-proxied callables call back through `_apply` via `bind(Class)`), and threads a `JvmRuntimeBuilder.JavaPrint` into the lisp-to-string builders so wrapped objects print `#<java <class>>` (BigInteger, and HashMap when hash tables are used, still fall through to `toString`). Template `.class` registered in `resource-config.json`, so the native binary can COMPILE `java:` programs; INTERPRETING them still fails there (no reflection metadata — even `(java:static "java.lang.Math" "max" 3 7)` → `No such class`) — verified. WASM stays a `Cannot compile: java:...` error, and there is still no `BuiltinFunctionWrappers` entry (`#'java:call` is a compile error; interpreter allows it). **Overload resolution is deterministic**: `select()` picks the constructor/method whose args marshal at the lowest total cost (`COST_EXACT` < `COST_WIDEN` < `COST_CONVERT` < `COST_NARROW` < `COST_BOXED` < `COST_PROXY`), ties broken by a stable signature string — never by `getMethods()` order. A single `marshal()` is the source of truth for both selection and conversion. `java:proxy` adapts a Lisp callable to an interface (dispatched as `(callable "method" arg...)`); a callable passed where an interface is expected is auto-proxied. **Sequence marshalling + varargs**: a proper list / rank-1 vector marshals to `T[]` (element-wise via `marshalSequence`, incl. primitive arrays) or, for any `List`-compatible target, to a `java.util.List` of `Object`-boxed elements; per-element costs count toward the overload total (so string elements prefer `String[]` over `Object[]`). A varargs executable is tried both as-is and with the tail packed (`tryVarargs`, flat `COST_VARARGS` penalty so a fixed-arity interpretation wins; the packed signature gets a `*` suffix to keep the tie-break total). Java **array** results unmarshal to Lisp lists (recursively); a returned `java.util.List` stays a `LispJavaObject`. A method declared in a JDK-internal class (e.g. a `List.of` result) is re-resolved to an accessible superclass/interface declaration (`accessibleMethod`). Symbols/hash-tables/dotted lists/rank-2 arrays are NOT marshalled. Tests: `JavaInteropTest` (interpreter) and `JvmJavaInteropCompilerTest` (compiled, mirrors the same cases — keep them in step; both headless JDK classes only — `Math`/`StringBuilder`/`ArrayList`/`Runnable`/`Supplier`/`Comparator`). Examples: `examples/java-interop.lisp`, `examples/swing.lisp` (reusable grid-window helper), `examples/life-gui.lisp`. Docs: `doc/{en,ja}/guides/java-interop.md` + five `reference/functions/java-*.md` (runnable examples use headless pure-compute calls only — never a GUI form, which would hang `DocExamplesTest`).
- **Template-class embedding (the `JavaBridgeTemplate` mechanism) is a LAST RESORT**: the normal way to give compiled output a runtime helper is, in order of preference, (1) a macro expansion into existing primitives (`LispMacroExpander` — no backend work at all), (2) a hand-assembled `Jvm/Wasm<Name>RuntimeBuilder` method (the standard used everywhere else, incl. the full reader and eval interpreters — self-contained, version-50, runs on any JRE 6+, byte-level control, no hidden constraints), and only then (3) embedding a project-compiled Java class ("template"): read its bytecode from the classpath, rename it into the default package via constant-pool rewrite (`JvmJavaRuntimeBuilder.renameClass`), base64-embed it, and `Lookup.defineClass` it at first use. Reach for (3) ONLY when the helper is genuinely too complex/error-prone to hand-assemble AND needs JDK facilities that are impractical in raw bytecode — `java:` interop qualifies on both counts (cost-based overload resolution, recursive marshalling, `Proxy` invocation handlers); nothing else in the code base currently does. Demerits to weigh before adding another template: (a) it silently raises the output's JRE floor — the template carries the project's class version (currently Java 25), so any program using the feature loses the version-50 "runs on any JRE 6+" property; (b) the template obeys invariants javac cannot check — no nested classes/records (each is a second class file the single-blob injection cannot carry; lambdas are fine), no imports of other rontolisp classes, and it must be written against the compiled value representation, duplicating logic that then has to be kept manually in sync with its interpreter twin; (c) the `defineClass` machinery is subtle — default-package requirement (Lookup.defineClass demands the caller's runtime package, hence the rename), lazy invokestatic resolution ordering (the `_javaInit` guard must run before the first bridge methodref executes), reflection back-calls needing `setAccessible`; (d) the base64 blob bloats every using program's constant pool and must be registered in `resource-config.json` for the native binary to compile the feature. If a new helper seems to need a template, first check whether the complex part can run at COMPILE time instead (like `LoadInliner` or the wasm-component blobs) or be expressed as a smaller hand-assembled runtime plus compile-time constants; when a template really is unavoidable, pin the rename with a round-trip test (see `JvmJavaInteropCompilerTest#renameClassLeavesOtherUtf8EntriesIntact`) and mirror the interpreter test suite against the compiled path.
