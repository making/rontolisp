...
- **`do`/`return`/`%block` + named `block`/`return-from`**: `return` is a non-local exit to the nearest enclosing `%block`, and works mid-expression on every backend (the JVM discards the abandoned operands). INTERPRETER: `block`/`return-from` are REAL named exits (BlockReturnSignal caught by the matching block; defun bodies = a block named after the function, defmethod bodies = one named after the generic; `%block` is transparent to the named signal, so a return-from crosses loops and closures within the dynamic extent). COMPILE PATH lite: `block` lowers to `%block` (name dropped) and `return-from` is rewritten to `return`, scoped to the nearest block (a deviation from CL). `.kb/do-return-block.md`
...
- **Dynamic (special) variables**: `defvar`/`declaim special` + `let` = a **shallow binding** (save/set/restore over the global cell), thread-scoped on the interpreter. Local `(declare (special x))` is honored pessimistically (program-wide); special-named lambda/macro parameters bind dynamically on the interpreter. Compile-path lite limits: `progv` = compile error, unwinding via `return` does not restore, `--no-gc` rejects specials. `.kb/dynamic-special-variables.md`
