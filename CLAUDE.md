...
- **`--component` (WASI 0.2 / Preview 2 output) for the WASM compiler**: Opt-in flag (CLI `--component`; `WasmLispCompiler(dynamic, component)` constructor; threaded as a `component` boolean). Default output stays a Preview 1 core module (no regression); when set, the output is a WASI 0.2 **component** runnable with `wasmtime run -W gc=y`. **Design decision -- least-invasive "adapter" wrapping**: rather than rewrite the print path to call WASI 0.2 streams directly, the rontolisp core module is emitted almost unchanged (it still imports the four `wasi_snapshot_preview1` functions and prints through `fd_write`) and those imports are satisfied by an adapter core module that implements `fd_write` on top of `wasi:cli/stdout` + `wasi:io/streams` (and stubs `fd_read`/`path_open`/`fd_close`, so reading and file I/O are unsupported in component mode for now). The only core-module changes in component mode (all in `WasmLispCompiler.compile`): (1) import the linear memory (`"mem"."memory"`) instead of defining+exporting it, (2) `_start`'s type becomes `()->i32` and its body returns `0`, exported as `run`. **Design decision -- shared canonical-memory module (avoids the lazy funcref trampoline)**: a lowered WASI import with a `list<u8>` parameter needs the canonical `(memory)` option, but that memory normally is the importing module's own -- circular, since the memory only exists after instantiation. `wasm-tools` resolves this with a lazy funcref-table trampoline; instead a tiny shared "memory" core module (exports `memory` + a bump `cabi_realloc`) is instantiated first, and both the canonical lowering and the rontolisp module use it, so lowering references an already-existing memory. The component assembly lives in `WasmComponentBuilder` (codegen.wasm), using `am.ik.wasm.ComponentWriter` (a new language-independent component-model binary encoder, companion to `WasmWriter`). Three fixed byte blobs are embedded as constants in `WasmComponentBuilder` because they are independent of the compiled program and impractical/needless to hand-build: the WASI import declarations (resource/variant/result/list type encodings, captured from a `wasm-tools`-generated reference) and the two helper core modules (memory + adapter); each was validated with `wasm-tools validate -f component-model` and run with `wasmtime run`. Component encoders are pinned by golden-byte tests in `ComponentWriterTest`; end-to-end component runs are covered by `WasmLispCompilerIntegrationTest` (Docker). README "Compile to a WASI 0.2 component" lists the limitations.
...
- `src/test/resources/ci-spec.yaml` is the single source of truth for the cross-backend end-to-end test (`CiSpecE2eTest`, package `am.ik.rontolisp.e2e`). Each case is `name` + `source` (Lisp) + `expected` (exact output lines, with an optional `expectedByBackend` override). The cases share global state and run IN ORDER, so the driver concatenates them into one program, runs the native binary once per backend (interpreter / JVM / WASM), and slices the output back per case — a failure names the exact case, not just a line number. Add a case here when adding a feature; there is no separate expected file to keep in sync. The driver only runs when `-Drontolisp.binary=<path>` is set (so `mvn test` on the JVM skips it); CI's `native-image` job runs it after building the binary.
