...
- **`tagbody`/`go` + `prog`/`prog*`**: INTERPRETER-ONLY special forms (`GoSignal` exception + label-indexed re-entry in `evalTagbody`); `prog` = `%block` + `let` + `tagbody`, so a user `(return x)` exits the prog. The compilers reject them (`.todo/146`).
- **jzon-driven CL additions (interpreter-first, compile path pending -- `.todo/146`)**: `shiftf`, `load-time-value` (lite: re-evaluates), `(setf (values ...))`, setf through a `(the T place)` wrapper, `typep` (literal type specifiers via the shared static type-test builder), `subtypep` (built-in lattice + class registry, single value), `mask-field`, `scale-float`, `%ieee754-*` bit primitives, fill-pointered/adjustable STRINGS (`LispString` carries the fill pointer; `make-array :element-type 'character` with `:fill-pointer`/`:adjustable` builds one), defstruct options (`:constructor`/`:conc-name`/`:predicate`/`:copier`), a runtime SYMBOL datum to `error` re-dispatches as a condition type (interpreter), `(symbolp nil)`/`(symbolp t)` = t (interpreter; compiled backends still answer nil -- divergence tracked), `case`/`ecase` keys match both the package-qualified and plain spelling.
