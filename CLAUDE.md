...
- **`rontolisp:http-handler` (incoming HTTP)**: handler takes a request plist and returns a response plist (headers included, both directions); interpreter (JDK `HttpServer`), JVM (generated class implements `HttpHandlerSupport.Handler`, needs the jar on the classpath), WASM `--component` (`wasi:http/handler@0.3.0`, a stackful-async lift whose response is delivered mid-task via `canon task.return`; run under `wasmtime serve -W gc=y -W exceptions=y -W component-model-async-stackful=y -W component-model-more-async-builtins=y`, wasmtime 46+). fetch + serve are ONE Lisp library (`http.lisp` over wit-imported `wasi:http@0.3.0`, spliced by `eval/HttpLibrary` following the reachable half). Preview-1 WASM = compile error. Spin cannot run it (no wasm-GC); wasmCloud hosts P3 experimentally (a `wasip3`-feature build targeting an RC -- interop unverified). `.kb/fetch-http.md`, `.todo/51`
