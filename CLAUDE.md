...
- **`java:` interop (interpreter-only Java reflection bridge)**: a fourth built-in package `java` (`LispNames.JAVA_PKG`, registered in `PackageRegistry`; does NOT use `cl`) with five functions — `java:new`/`java:call`/`java:static`/`java:field`/`java:proxy`. Wrapped host objects are a new `LispVal` permittee `LispJavaObject(Object ref)` (prints `#<java <class>>`). The bridge is `eval/JavaInterop.java`; wired in `LispEvaluator.registerJava()` (registered alongside eval/load because `java:proxy` applies a user callback via the evaluator's `apply`). **JVM-interpreter-only by design** (runs only under `java -jar rontolisp.jar`): a `LispJavaObject` cannot be lowered, so compiling a `java:` form is a `Cannot compile: java:...` error (no JVM/WASM compiler classes, no `BuiltinFunctionWrappers` entry); and the reflection needs runtime metadata a GraalVM native image does not carry, so it fails in the native binary too (even `(java:static "java.lang.Math" "max" 3 7)` → `No such class`) — verified. Adding `LispJavaObject` to the sealed `LispVal` only required a new case in `LispEvaluator`'s eval switch — the `Jvm/WasmQuoteCompiler` switches have a `default` that throws (a java object never appears in source). **Overload resolution is deterministic**: `select()` picks the constructor/method whose args marshal at the lowest total cost (`COST_EXACT` < `COST_WIDEN` < `COST_CONVERT` < `COST_NARROW` < `COST_BOXED` < `COST_PROXY`), ties broken by a stable signature string — never by `getMethods()` order. A single `marshal()` is the source of truth for both selection and conversion. `java:proxy` adapts a Lisp callable to an interface (dispatched as `(callable "method" arg...)`); a callable passed where an interface is expected is auto-proxied. **Sequence marshalling + varargs**: a proper list / rank-1 vector marshals to `T[]` (element-wise via `marshalSequence`, incl. primitive arrays) or, for any `List`-compatible target, to a `java.util.List` of `Object`-boxed elements; per-element costs count toward the overload total (so string elements prefer `String[]` over `Object[]`). A varargs executable is tried both as-is and with the tail packed (`tryVarargs`, flat `COST_VARARGS` penalty so a fixed-arity interpretation wins; the packed signature gets a `*` suffix to keep the tie-break total). Java **array** results unmarshal to Lisp lists (recursively); a returned `java.util.List` stays a `LispJavaObject`. A method declared in a JDK-internal class (e.g. a `List.of` result) is re-resolved to an accessible superclass/interface declaration (`accessibleMethod`). Symbols/hash-tables/dotted lists/rank-2 arrays are NOT marshalled. Tests: `JavaInteropTest` (headless JDK classes only — `Math`/`StringBuilder`/`ArrayList`/`Runnable`/`Supplier`/`Comparator`). Examples: `examples/java-interop.lisp`, `examples/swing.lisp` (reusable grid-window helper), `examples/life-gui.lisp`. Docs: `doc/{en,ja}/guides/java-interop.md` + five `reference/functions/java-*.md` (runnable examples use headless pure-compute calls only — never a GUI form, which would hang `DocExamplesTest`).
