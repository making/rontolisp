...
- **`--simd` on the interpreter (opt-in Vector API)**: `rontolisp prog.lisp --simd` (no `-o`) overrides the seven vectorizable `vec.lisp` defuns with `jdk.incubator.vector` natives (`eval.VecSimd` + `eval.VecSimdKernels`, lane-for-lane mirrors of `JvmSimdVectorTemplate` over the interpreter's header-less packed arrays -- `eval` may not depend on `codegen.jvm`). The DEFAULT interpreter keeps the scalar `vec.lisp` (the cross-backend byte-identity oracle; ci-spec never passes `--simd`). Absent module (`java -jar`) = a note + scalar fallback, not a failure. The native profile bakes the module in with `-H:+VectorAPISupport`, which **cannot** coexist with `-H:+SharedArenaSupport`, so `JLineRepl` pins JLine's `jni` terminal provider in the image. Web Image cuts it out via `Target_VecSimd`. Details: `.kb/vec.md` (acceleration layer 0).
- **`--simd` on wasm-GC (native v128 + linear-memory packed arrays)**: `rontolisp prog.lisp -o prog.wasm --simd` routes the twelve vectorizable `vec:` kernels to emitted v128 runtime helpers (`WasmVecSimdRuntimeBuilder`, called from `WasmVecSimdCompiler` at the call site like the JVM bridge). Because `v128.load`/`store` address LINEAR memory, `--simd` also switches the packed float-array repr: `TYPE_FARRAY`'s `data` field holds an `i31ref` pointer into a bump-allocated linear block (`[count][kind][pad][pad][elements]`) instead of a `TYPE_F64ARR`/`TYPE_F32ARR` GC array. The data field is `(ref null eq)` either way, so **the type section is unchanged** (component blobs untouched); the repr is chosen at compile time, so one module holds exactly one of them. The thirteen emitted functions (12 kernels + `_vec_alloc`) sit at `FUNC_VEC_BASE` and shift `FUNC_USER_BASE` -- read it via `userFuncBase()` -- so a build WITHOUT `--simd` is byte-identical to one that never knew the flag. The arena starts lazily at `memory.size() << 16` (above every static page, so `--component` works) and `memory.grow`s; nothing frees it within a run, hence the `-into` kernels. `matvec` (GEMV) works here (the struct carries `dims`), unlike `--no-gc`. Details: `.kb/vec.md` (acceleration layer 3).
- **Destination-passing `vec:` kernels (`vec:add-into` &c)**: every vector-returning kernel has an `-into` sibling (`add`/`sub`/`mul`/`scale`/`matvec`) that writes into a caller-supplied destination (argument 1, CL's `map-into` order) and returns THAT value, so a hot loop allocates nothing. This exists because the two **v128-emitting WASM targets** (`--no-gc`, and wasm-GC under `--simd`) put packed arrays in linear memory, bump-allocated with **no free** (`--no-gc` reclaims only by an arena pop at an export boundary -- todo-88/89 -- so nothing is freed *within* one call; wasm-GC `--simd` has no export boundary at all); packed arrays on scalar wasm-GC/JVM/interpreter are GC objects and are reclaimed (measured flat). With `-into` the bump high-water equals the live set; on the GC'd backends it is only an allocation-rate optimization. `.todo/101` would give wasm-GC `--simd` the `--no-gc` memory behavior, which is why it should gate the linear repr behind `--simd`. Three invariants: (1) the element-wise kernels MAY alias `out` with an operand, `matvec-into` may NOT (each output element folds over all of `x`) -- (2) that alias guard is written THREE times (the `vec.lisp` `eq` check, `VecSimd`, `JvmSimdVectorTemplate`) because each accelerated call site REPLACES the defun; (3) both WASM `--simd` paths lower `-into` by threading `boolean into` through the existing kernels, skipping the block allocation and using the caller's block as the destination pointer -- the loop bodies are byte-identical, and `matvec-into` joins `matvec` in `--no-gc`'s `SIMD_UNSUPPORTED_NO_GC` (wasm-GC `--simd` ships GEMV). Details: `.kb/vec.md`; follow-up `.todo/104` (a real free: `with-arena` over the one bump word).
