...
- **`do`/`return`/`%block`**: `return` is a non-local exit to the nearest enclosing `%block`, and only works where the operand stack is empty. `return-from` is lite -- rewritten to `return`, scoped to the **nearest enclosing function** (a deviation from CL). `.kb/do-return-block.md`
...
- **`rontolisp:http-handler` (incoming HTTP)**: handler takes a request plist and returns a response plist; interpreter (JDK `HttpServer`), JVM (generated class implements `HttpHandlerSupport.Handler`, needs the jar on the classpath), WASM `--component` (`wasi:http/incoming-handler`, run under `wasmtime serve -W gc=y`; headers dropped). Preview-1 WASM = compile error. Spin cannot run it (no wasm-GC). `.kb/fetch-http.md`, `.todo/51`
...
- **`rontolisp:wit-export` / `--scaffold-wit`** (todo 126): a program declares the WIT world it implements; the world is then the AUTHORITATIVE export list (a hand-written `wasm-export` or a `rontolisp:http-handler` beside it is a compile error), every `defun` is checked against it (name/arity/type/`async`, errors naming the WIT line), and it LOWERS into the equivalent `wasm-export` directives -- so the component is byte-identical and there is no new export path. Only the export side; a world's imports are declared with `rontolisp:wit-import` (below) and, for a component, `.todo/128`. `.kb/wit.md`
- **`rontolisp:wit-import` / `wit-provide`** (todo 127): a program declares the WIT interface it CALLS, and the directive LOWERS per backend -- Preview 1 WASM = one `wasm-import` per WIT function (byte-identical to the hand-written block, still shakeable under `--optimize`); interpreter/JVM = a synthesized `defpackage` + one ORDINARY `defun` per function dispatching through the interface's provider; `--component`/`--no-gc` = clear error. **The core ships NO provider for any concrete interface** -- it knows the provider MECHANISM only (`rontolisp:wit-provide` binds a plain Lisp callable; an unbound interface signals `rontolisp:wit-error`; a `wit-provide` is dropped as inert on the WASM backends, where the host is the provider). An implementation of a WIT interface is ordinary USER code -- a new host interface must cost a `.wit` file, not core code (`examples/wit-keyvalue` implements `wasi:keyvalue/store` twice, in Lisp and over `java:` interop). The runtime is `wit.lisp` (a Lisp-source library, not prunable), so no backend gained a case. `WitImportInliner` runs BEFORE `UserMacroExpander` (its synthesized `defpackage` must exist before any call site resolves) -- the opposite of `WitExportInliner`. `.kb/wit.md`
