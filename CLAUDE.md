...
- **`rontolisp:http-handler` (incoming HTTP / serving)**: `(rontolisp:http-handler 'name [port])` serves requests with a handler taking a request plist (`:method`/`:path`/`:headers`/`:body`) and returning a response plist (`:status`/`:headers`/`:body`) -- the incoming counterpart of `fetch`, same value model. **Interpreter-only so far** (`HttpHandlerSupport`: blocking JDK `HttpServer`, one virtual thread per request; `serve()` blocks, `start()` is the test seam; registered in `LispEvaluator` because serving applies the handler via `apply`). JVM and the WASI-component path (compile to a `wasi:http/incoming-handler` component for `wasmtime serve`/Spin) are IN PROGRESS -- both currently a clear compile error ("interpreter backend"). The WASM pipeline is proven end-to-end and fully specified (adapter WAT calling a wasm-export `%http-dispatch`, `buildServe` wiring, derived wasi:http@0.2 ABI signatures) in `.todo/51-wasi-http-incoming-handler-spin.md`. Details: `.kb/fetch-http.md`.
