...
variant (WASI 0.2 `wasi:http`/`wasi:io` hybrid, `import-block-http.bin` +
`adapter-http.wat`) while tcp needs the sockets variant (pure WASI 0.3,
`import-block-sock.bin` + `adapter-sock.wat`), and no combined variant exists.

The path is mechanical: a fourth WIT world (`uni-http` + `wasi:sockets/types@0.3.0`
appended last), a merged adapter (adapter-http.wat's fetch machinery + the real
socket table/exports from adapter-sock.wat, replacing the four errno stubs), a
