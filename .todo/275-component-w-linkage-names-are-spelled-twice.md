...
After todo-273 a print-only WASI 0.3 component is 1,820 B:

| part | bytes |
| --- | ---: |
| shaken core module | 653 (`.todo/271` owns ~104) |
| preview1 adapter | 547 |
| import block (`wasi:cli/types` + `wasi:cli/stdout`) | 197 |
| shared memory module | 34 |
| aliases / canon / instances / exports | 389 |
...
One character each would leave 9 x 2, saving ~232 B -- **12.7% of the whole
component**, which would take hello from 1,820 to roughly 1,588.
...
delete all of it (1,820 -> ~1,541, measured by hand on the emitted component) and
...
- The core module (`.todo/271`) and the encoding of what is emitted
  (`.todo/274`); this is only about the linkage names.
