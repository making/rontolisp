...
| ports (`current-output-port`, string ports, bytevector ports and `read-u8`/`write-u8`, `read-line`, `read-char`) | `%STREAM` instances (`.kb/read-load-streams.md`) | `display`/`write` take one argument today; the two-argument form is an arity error. `read` from the current input port is split off: `.todo/832` |
