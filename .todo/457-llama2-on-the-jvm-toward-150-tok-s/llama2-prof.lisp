;;; --- timers (instrumented copy) ---
(defvar *tm* (make-array 16 :element-type 'double-float :initial-element 0.0))
(defun now () (java:static "java.lang.System" "nanoTime"))
(defun tick (k t0) (setf (aref *tm* k) (+ (aref *tm* k) (- (now) t0))))
  ...
  (let* ((n-heads (getf model :n-heads))
    ...
    (dotimes (l (getf model :n-layers))
      ...
      (let* ((xb (rmsnorm x (aref (getf model :rms-att) l)))
             (q (progn (tick 0 t0) (setq t0 (now)) (vec:matvec (aref (getf model :wq) l) xb)))
             (k (vec:matvec (aref (getf model :wk) l) xb))
             (v (vec:matvec (aref (getf model :wv) l) xb)))
        (tick 1 t0) (setq t0 (now))
        (rope q n-heads hs pos rope-cos rope-sin)
        (rope k n-kv hs pos rope-cos rope-sin)
        (tick 2 t0) (setq t0 (now))
        (let ((a (attention model state l q k v pos)))
          (tick 3 t0) (setq t0 (now))
          (setq x (vec:add x (vec:matvec (aref (getf model :wo) l) a)))
          (tick 4 t0)))
      (setq t0 (now))
      (let* ((xb (rmsnorm x (aref (getf model :rms-ffn) l)))
             (h1 (progn (tick 5 t0) (setq t0 (now)) (vec:matvec (aref (getf model :w1) l) xb)))
             (h3 (vec:matvec (aref (getf model :w3) l) xb)))
        (tick 6 t0) (setq t0 (now))
        (let ((hh (vec:mul (silu h1) h3)))
          (tick 7 t0) (setq t0 (now))
          (setq x (vec:add x (vec:matvec (aref (getf model :w2) l) hh)))
          (tick 8 t0))))
    (setq t0 (now))
    (let ((r (vec:matvec (getf model :wcls) (rmsnorm x (getf model :rms-final)))))
      (tick 9 t0)
      r)))
...
(defun generate (model state tokenizer prompt steps)
  (let* ((pieces (first tokenizer))
    ...
    (loop while (< pos steps)
          do
            (let* ((tf (now))
                   ...
                   (next (progn (tick 14 tf) (setq tf (now)) (if rest (pop rest) (sample logits)))))
              (tick 15 tf)
              (setq pos (+ pos 1))
    ...
    (let ((names '("rmsnorm-att" "qkv gemv" "rope" "attention" "wo gemv+add" "rmsnorm-ffn" "w1/w3 gemv" "silu*h3" "w2 gemv+add" "final rmsnorm+cls" "  att: kv append" "  att: qh copy+scores gemv" "  att: softmax" "  att: oh gemv+copy" "FORWARD total" "sample+decode")))
      (dotimes (k 16)
        (format *error-output* "~28a ~8,1f us/token~%" (nth k names) (/ (aref *tm* k) (* 1000.0 pos)))))))
