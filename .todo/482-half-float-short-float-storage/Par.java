...
public class Par {
	...
	static final java.util.concurrent.ExecutorService POOL = java.util.concurrent.Executors.newFixedThreadPool(THREADS);

	static void par(int rows, java.util.function.BiConsumer<Integer, Integer> body) {
		int chunk = (rows + THREADS - 1) / THREADS;
		java.util.List<java.util.concurrent.Future<?>> fs = new java.util.ArrayList<>();
		for (int tI = 0; tI < THREADS; tI++) {
			int from = tI * chunk, to = Math.min(rows, from + chunk);
			if (from >= to) break;
			fs.add(POOL.submit(() -> body.accept(from, to)));
		}
		for (var f : fs) try { f.get(); } catch (Exception ex) { throw new RuntimeException(ex); }
	}

	static void gemvF32P(float[] w, int rows, int cols, float[] x, float[] r) {
		par(rows, (from, to) -> {
			for (int row = from; row < to; row++) {
				...
				for (; i < b; i += FS.length())
					acc = acc.add(FloatVector.fromArray(FS, w, base + i).mul(FloatVector.fromArray(FS, x, i)));
				r[row] = acc.reduceLanes(VectorOperators.ADD);
			}
		});
	}

	static void gemvF16P(short[] w, int rows, int cols, float[] x, float[] r) {
		par(rows, (from, to) -> {
			for (int row = from; row < to; row++) {
				int base = row * cols; FloatVector acc = FloatVector.zero(FS);
				int i = 0, b = IS.loopBound(cols);
	...
	public static void main(String[] a) {
		...
		for (int[] sz : new int[][] { { 288, 288 }, { 1024, 1024 }, { 4096, 4096 }, { 8192, 8192 } }) {
			...
			int it = e > (1 << 22) ? 4 : 50;
			long t32 = t(() -> gemvF32P(wf, rows, cols, x, r1), it);
			long t16 = t(() -> gemvF16P(wh, rows, cols, x, r2), it);
			double err = 0, mx = 0;
