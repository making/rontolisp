...
more -- `examples/wasm-size/pi_approx` is 3,540 bytes at `--optimize` and 3,420 at
`--optimize=size`, against the 6,034-byte floor of the cross-language table -- so
this is margin, and margin is what stops one regression from losing the row again.
...
| probe | bytes |
| --- | ---: |
| the loop + `(princ "done")` | 2,547 |
| the loop + `(princ <the f64 result>)` | 6,312 |

Printing ONE float is **+3,765 bytes** -- nearly four times what the whole
`~,15F` rendering costs (993), and larger than the entire rest of the module. It is `_princ_val`'s generic dispatch plus
...
The empty `(dotimes (i 1000000))` was 3,060 bytes on its own before the shared
coercion helper landed; re-measure it first. The bounds are
...
For whoever picks this up, the 3,540-byte module's own breakdown (3,119 of it
function bodies):

| | bytes |
| --- | ---: |
| `_fixed_dec` (the whole `~,nF` renderer) | 457 |
| the program's top-level body (the loop) | 331 |
| `_write_stream_str` | 253 |
| `_as_f64` (the shared coercion, 26 inline copies before it) | 80 |
| everything else (arithmetic tower, I/O, prologue) | ~2,000 |

Item 2 attacks the loop row, which is now small enough that the remaining bulk is
the arithmetic tower rather than the caller. Item 1 does not appear here at all --
the program no longer prints a float -- but any program that DOES print one pays
it, and it is now the single biggest identified block anywhere.

`.todo/276`'s coercion-helper items (3 and 4) landed 2026-08-07 and are what took
this module from 5,356 to 3,540; its arity-dispatcher items (1 and 2) are still
open and are the larger remaining lever.
