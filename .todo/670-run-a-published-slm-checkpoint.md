...
**One ordering constraint across the two sides -- now DISCHARGED**: `.todo/692`
(`widen-float-bits` traps into a `--simd` wasm destination) had to land before
`.todo/488`'s interception, since 488's wiring is what puts bf16 through `--simd` and it
should not be laid over a known trap on that path. **692 closed 2026-09-03**, so 488 is
unblocked and lane A1 starts at `678` instead.
