...
### Phase 5b handoff — linalg → packed double-float FOR SPEED (READ FIRST)
...
generic arithmetic, and the current "exact arithmetic" wording in `guides/linear-algebra.md` is
post-hoc ("結果論"), NOT a binding contract. **linalg's priority is SPEED.** So the earlier
"a full migration is WRONG / preserve exactness" recommendation is REVERSED: **migrate linalg to
the packed `double-float` representation** and accept that its output changes int/ratio → double
(`(linalg:eye 3)` → `#2A((1.0 0.0 0.0) ...)`, `(linalg:inv #2A((1 2)(3 4)))` → floats,
`(linalg:solve ...)` → floats, `(linalg:det ...)` → a double). This is numpy's own model.

**Numerical note to keep in mind (not a blocker):** float `det`/`inv`/`solve` lose exactness —
a singular integer matrix's determinant becomes a tiny epsilon instead of exactly `0`. That is
expected float behavior (numpy has it) and acceptable given the speed priority; just do not assert
exact-zero singularity in the updated tests.

**What ALREADY works (verified):** a packed `#f(...)` interoperates with linalg as an INPUT today —
`(linalg:add #f(1.0 2.0 3.0) #f(4.0 5.0 6.0))` → `#(5.0 7.0 9.0)`, `(linalg:dot ...)`,
`(linalg:matmul #f((1.0 2.0)(3.0 4.0)) #f(...))` all work (aref-based kernels read a packed array
transparently). What's missing is packed OUTPUTS (the constructors still build general arrays) +
the kernel acceleration.

**Implementation plan:**
1. **`linalg.lisp` produces packed doubles.** Change every constructor / result `make-array` from
   `:initial-element 0` (int) to `:element-type 'double-float :initial-element 0.0` (mechanical;
   arithmetic then flows as float since `aref` returns doubles). Verify no linalg op stores a
   non-real or depends on ratio arithmetic (it should not — it is numeric throughout). Intermediate
   lists (reshape/to-list) stay lists; only the array outputs become packed.
2. **Reconcile linalg docs + tests to doubles (the bulk of the churn, but mechanical).**
   - `guides/linear-algebra.md` (en+ja): rewrite the "Arithmetic is generic and exact" paragraph
     (now float, numpy-style) and update every worked-example output to doubles.
   - Per-operator pages `doc/{en,ja}/reference/functions/linalg-*.md`: auto-rewrite the `; =>` /
     output annotations with `./mvnw -Drontolisp.doc.fix=true -Dtest=DocExamplesTest#fixDetailResults test`,
     then `./mvnw -Dtest=DocExamplesTest test` to verify. Mirror en↔ja.
   - `LinalgLibrary`/linalg unit + e2e tests + any `linalg` `ci-spec.yaml` case → doubles; rerun the
...
**`--no-gc` limit:** linalg matrices are rank-2, and `--no-gc` packed is rank-1-only (Phase 4A:
rank>1 = compile error) + has no lists (reshape/to-list). So linalg on `--no-gc` stays limited to
rank-1 float-vector ops at most; matrices are JVM/interp/wasm-GC only. Do not chase full linalg on
`--no-gc`.
