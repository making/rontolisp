	...
	public static void main(String[] args) throws Exception {
		...
		for (int s = 0; s < shapes.length; s++) {
			...
			for (int g : new int[] { 1 << 12, 1 << 13, 1 << 14 }) {
				int floor = Math.max(1, g / n);
				String gs = " 2^" + Integer.numberOfTrailingZeros(g);
				int cap8 = Math.max(floor, d / (8 * threads));
				int cap4 = Math.max(floor, d / (4 * threads));
				int cap2 = Math.max(floor, d / (2 * threads));
				report("cc-tree" + gs, () -> ccTree(pool, w, x, r, d, n, floor), reps, tser, ref, r);
				report("adapt-tree" + gs, () -> adaptTree(pool, w, x, r, d, n, floor), reps, tser, ref, r);
				report("adapt-tree" + gs + " cap8", () -> adaptTree(pool, w, x, r, d, n, cap8), reps, tser, ref, r);
				report("adapt-tree" + gs + " cap4", () -> adaptTree(pool, w, x, r, d, n, cap4), reps, tser, ref, r);
				report("adapt-tree" + gs + " cap2", () -> adaptTree(pool, w, x, r, d, n, cap2), reps, tser, ref, r);
			}
			System.out.println();
		}
		pool.shutdown();
