...
the whole overlap identical -- 581 characters / 585 UTF-8 bytes, measured over a STATED window (both sides
run to the same prompt and compared from the first generated byte to the end of the
shorter output; ~143 tokens, approximate because re-encoding decoded text through a
byte-level BPE need not reproduce the sequence that generated it, so only the character
count is exact) -- where `.todo/677`'s
...
Load, re-taken on the shipped tree (`20ccb559` merged, i.e. after `.todo/690`): 9.0-9.8 s
from the GGUF (its tokenizer 0.32-0.34 s), 8.9-9.1 s from the safetensors (`tokenizer.json`
+ KV cache 1.54-1.67 s). **`.todo/690` did not change either figure**, which is the
expected result and worth recording: its headline is `rontolisp:json-parse` going from 180 s
unfinished to 525 ms on a 13 MB `tokenizer.json`, but `llama2.lisp` was never paying that --
it carried its own byte-level JSON reader precisely because `json-parse` could not finish,
and 690 deleted that reader in favour of the now-fast builtin at equal cost (425 ms against
447 ms measured on the same file). A model load is unchanged; what 690 buys is that the
special-case reader is gone.
