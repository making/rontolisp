...
| 2 | `asdf:*user-cache*`, `asdf:registered-systems`, `asdf:component-name`/`-children`/`-sideway-dependencies`, `asdf:system`/`package-inferred-system`/`cl-source-file`/`module` as `typecase` types AND `defmethod` specializers, `find-system` -> a system OBJECT, a runtime `load-system` of an already-loaded system | DONE (2026-08-15, `.todo/374`): the component metaobjects are real CLOS classes on all four backends (`asdf.lisp` / `AsdfRuntimeLibrary`), `find-system` answers a memoized instance, the readers + `registered-systems` + `*user-cache*` exist, a nested `load-system` of a spliced system is a nil no-op, and `asdf:test-system` drives the recorded `:perform`/`:in-order-to` test-op wiring (`.kb/asdf.md`) | -- |
| 3 | `*load-pathname*` bound to the file being loaded when a `deftest` runs (rove's file -> package map, `make-new-suite`, is what `run` walks for a plain `defsystem` test system) | DONE (2026-08-15, `.todo/375`): the variable holds the file on every backend -- the interpreter binds it, the compile paths assign it per spliced file from `LoadInliner`'s `%begin-file` brackets -- and a COMPONENT is loaded by its resolved path, so it equals `asdf:component-pathname` byte for byte, which is the correlation `run` walks (`.kb/load-inliner.md`, `LoadContextE2eTest`). `(rove:run "my-app/tests")` over a plain `defsystem` test system runs its suites on the INTERPRETER now, absolute or relative system path alike | -- |
...
| 12 | `(let ((*print-case* :downcase)) (princ-to-string name))` -> "add-test" (test names, `run*` patterns) | DONE (2026-08-15, `.todo/041`): `*print-case*` converts every symbol the printer spells -- `princ`/`prin1`/`print`/the three `-to-string`s/`write`/`~A`/`~S`/`~W`, and a `#'`-reference of them -- through ONE shared prelude renderer (`%print-cased`) on all four backends, text-identical to SBCL 2.2.9; a program that never names the variable is byte-identical to before (`.kb/pretty-printer.md`) | -- |
| 13 | `tree-equal` (`expands`), `remprop` (`remove-test`), `enough-namestring` + `*default-pathname-defaults*` + `pathname-device` (source-location printing, dead code today but a compile error for the special), `(setf uiop:getenv)` (`with-local-envs`, i.e. `run`'s `:env` -- a hard "setf does not support place" the moment `run` compiles), `uiop:implementation-identifier`/`featurep`/`pathname-parent-directory-pathname`/`lispize-pathname` (`resolve-file`'s fasl arm, unreachable while `asdf:*user-cache*` is nil), `uiop:quit` (a test runner's exit code) | **the pathname three landed 2026-08-15 with the `.todo/036` close-out**: `enough-namestring`, `pathname-device` (nil, which is exactly what `resolve-file` wants) and the special `*default-pathname-defaults*` (`#P""`, a genuine dynamic variable) are real on all four backends, so `(ql:quickload "rove")` now COMPILES -- verified end to end, the JVM class loads rove and runs. **`remprop` landed 2026-08-15 with the `.todo/038` close-out** (one prelude defun over the `%symbol-plists` store, all four backends). **`uiop:ensure-absolute-pathname` / `absolute-pathname-p` / `compile-file-type` landed with `.todo/375`**: the spike note "reached only when `*load-pathname*` is a fasl path" was wrong -- `resolve-file` calls the first two on EVERY `deftest` with a non-nil `*load-pathname*`, so a stub there is a hard error. **the whole `uiop/pathname` family (50/50) landed 2026-08-15 with the `.todo/357` close-out**, so `uiop:pathname-parent-directory-pathname` and rove's `enough-namestring*` inputs are all real. **`tree-equal` landed 2026-08-15 with the `.todo/033` close-out** (a prelude defun with `:test`/`:test-not`, all four backends), so `expands` compares macroexpansions for real. **the whole `uiop/os` family (22/22) landed 2026-08-15 with the `.todo/356` close-out**: `(setf uiop:getenv)` is a real setf place -- no backend can rewrite its own environment, so the write records a per-program OVERRIDE that `getenv` reads back first, one definition on all four backends -- and `implementation-identifier`/`featurep` (plus `architecture`, `operating-system`, `os-unix-p` and the rest) answer for the backend that is running, derived from `featurep` over the target's `*features*` (`.kb/uiop.md`, `uiop/os`'s decisions). **`uiop:quit` landed 2026-08-15 with the `uiop/image` exit half** (`.todo/362`): one Lisp definition over a `%host-exit` primitive -- an exit signal `RontoLispCli.main` turns into the process code on the interpreter, `System.exit` on the JVM, `proc_exit` / wit-imported `wasi:cli/exit@0.3.0` on the wasm pair -- so `(uiop:quit (if (rove:run :my-app/tests) 0 1))` really ends the process with 0 or 1 on all four backends, and neither an enclosing `handler-case` nor an `unwind-protect` cleanup interferes (`.kb/uiop.md`). What is left here is `uiop:lispize-pathname` (`uiop/lisp-build`, `.todo/365`) | ~~`.todo/033`~~, ~~`.todo/036`~~/~~`357`~~, ~~`.todo/356`~~, ~~`.todo/362`~~ (the exit half; `362` now carries the command line only) |
| 14 | `(make-instance (intern "SPEC-REPORTER" package) :stream s)` (`make-reporter`) | **the compile-path half landed 2026-08-16 (`.todo/382`)**: a RUNTIME class designator matches BOTH colon spellings on every generated dispatch (`%find-class`, `%mop-make-instance`, `%allocate-instance`, the runtime `typep`/`subtypep` tables, the condition-class arm of `error`), so the call resolves a NON-exported class too -- one helper, `LispMacroExpander.addDesignatorSpellings` (`.kb/clos.md`). DONE (2026-08-16, `.todo/383`): the interpreter no longer double-qualifies the foreign-package class name `type-of` interns -- an already-qualified canonical spelling names the symbol it spells (`PackageResolver.internSpelling` -> `resolveQualified`), so `(eq (type-of x) 'pkg:class)` is true on all four backends and `type-of` agrees with `class-name` (`.kb/symbol-runtime-api.md`) | ~~`.todo/382`~~, ~~`.todo/383`~~ |
...
argument-inspecting arm disabled (row 8, no longer needed); `enough-namestring*` = `namestring` (row 13: `*default-pathname-defaults*` -- no
longer needed either). Undo
...
5. ~~`.todo/374` (High)~~ DONE + ~~`.todo/375` (Medium)~~ DONE + ~~`.todo/036` (Medium)~~ DONE -- `(rove:run :my-app/tests)` and `(asdf:test-system "my-app")`. The interpreter runs a plain-`defsystem` test system end to end, and rove now COMPILES on the JVM/WASM too (row 13's two blocking names are real).
6. ~~Row 12 (`.todo/041`, output parity with SBCL)~~ DONE. ~~Row 14~~ DONE; row 13 is down to `uiop:lispize-pathname` (`.todo/365`) and can trail.
...
- Docs: a row in `doc/{en,ja}/guides/asdf-systems.md` "What can I actually load?"
  ...
  four run commands, and the exit-code recipe -- `(uiop:quit (if (rove:run
  :my-app/tests) 0 1))`, real on all four backends since `.todo/362`'s exit half
  (`reference/uiop/image.md`); `.kb/asdf.md` gets the rove paragraph in the
  loadable-libraries record.
