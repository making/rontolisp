...
2. ~~`.todo/255` (High)~~ DONE + `.todo/376` (Low) -- `deftest` registers under the right suite and `run-suite`/`package-suite` find it. After these two, `(rove:run-suite *package*)` is the first working entry point.
