...
| 14 | `(make-instance (intern "SPEC-REPORTER" package) :stream s)` (`make-reporter`) | **the compile-path half landed 2026-08-16 (`.todo/382`)**: a RUNTIME class designator matches BOTH colon spellings on every generated dispatch (`%find-class`, `%mop-make-instance`, `%allocate-instance`, the runtime `typep`/`subtypep` tables, the condition-class arm of `error`), so the call resolves a NON-exported class too -- one helper, `LispMacroExpander.addDesignatorSpellings` (`.kb/clos.md`). What is left is the interpreter `type-of` double-qualifying a foreign-package class name | ~~`.todo/382`~~, `.todo/383` |
...
6. ~~Row 12 (`.todo/041`, output parity with SBCL)~~ DONE. Rows 13-14 are polish and can trail.
