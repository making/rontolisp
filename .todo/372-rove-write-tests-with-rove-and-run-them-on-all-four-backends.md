...
argument-inspecting arm disabled (row 8, no longer needed); `enough-namestring*` = `namestring` (row 13: `*default-pathname-defaults*`). Undo
...
5. ~~`.todo/374` (High)~~ DONE + ~~`.todo/375` (Medium)~~ DONE -- `(rove:run :my-app/tests)` and `(asdf:test-system "my-app")`. The interpreter runs a plain-`defsystem` test system end to end; the COMPILE paths still stop at row 13's remaining three names.
