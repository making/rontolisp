...
- **2026-09-16, `.todo/037` closed** -- the number-extensions checklist is
  complete (every operator it lists shipped; `.todo/818`/`.todo/819` closed the
  last two). The numbers 218 + misc 25 residue was re-measured byte-identical
  and every 037-adjacent name classified: fourteen are the universe cascade
  (`*MINI-UNIVERSE*`/`*UNIVERSE*` unbound, §1 out of scope, including
  `IMAGPART.ERROR.3`), `LDB.PLACE.ORDER.1` is a setf evaluation-order gap
  (`.todo/001`/`.todo/041`), and the bulk (`PARSE-INTEGER`, `BOOLE`,
  `UPGRADED-COMPLEX-PART-TYPE`, `EXPT`/`EXP`, `RANDOM-STATE`, `ATAN`, ...)
  belongs to other families. 0 fixed, 0 regressed on close (verification only,
  no code change).
...
- **2026-09-16, `.todo/819` `imagpart` signed zero** -- CLHS `imagpart` of a
  real IS `(* 0 number)`, so a negative float answers `-0.0` (SBCL); the
  interpreter/JVM/WASM-GC answered constant `+0.0` and now multiply by `0.0`
  instead (unbox + `DMUL`/`f64.mul` + box on the compiled paths), while
  `--no-gc` still refuses the complex family outright. Deliberately not an
  `eql` change: CLHS `eql` returns false for distinct zeros, SBCL answers NIL,
  `.kb/linalg-simd.md` pins `(eql nz pz)` NIL. **numbers 219 -> 218
  (`IMAGPART.4`), misc 26 -> 25 (`MISC.598`, explicitly PASS after),
  0 regressed**, as a diff of failing test NAMES. Pinned by
