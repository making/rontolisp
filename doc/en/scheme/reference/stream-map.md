...
Returns the stream of `proc` applied element-wise to the given streams, computed lazily, so it works on infinite streams. With several streams it ends when the shortest one does. A *[Structure and Interpretation of Computer Programs](../sicp.md)* (SICP)/MIT name, not R7RS: no library exports it, so it is visible only to a file with no `import` and at the REPL.
