...
- Outgoing HTTP (`rontolisp:fetch` / `rontolisp:await`) works in component mode, including true asynchrony: `fetch` sends the request and returns a promise (the in-flight `wasi:http` response handle) immediately, so several requests can overlap before `await` blocks on each. It is a **hybrid**: the base I/O stays WASI 0.3 while fetch itself imports `wasi:http@0.2` + `wasi:io@0.2` (async `wasi:http@0.3` does not exist upstream yet — see `.todo/02-upgrade-fetch-to-wasi-http-0.3.md`). Run a fetch component with `-S http=y` in addition to the async flags. Non-fetch components do not import `wasi:http`, so they do not need `-S http`.
