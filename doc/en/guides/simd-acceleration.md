...
The whole engine is [`examples/llama2/llama2.lisp`](https://github.com/making/rontolisp/blob/develop/examples/llama2/llama2.lisp): llama2.c's `run.c` ported to one file -- checkpoint loader, tokenizer, forward pass, sampler -- over the real TinyStories checkpoints, telling the same stories as the C program token for token. Its 15 million weights load through `read-sequence` over packed single-float arrays, and its decode is over a hundred `vec:matvec`s per token; on stories15M, `--simd` takes the JVM from 66 to 218 tokens/s and wasm-GC from 0.4 to 128, all on one thread (`run.c -O2`, one thread: 147; measured 2026-08-22 on this project's NVIDIA GB10 box). See [its README](https://github.com/making/rontolisp/blob/develop/examples/llama2/README.md).
