...
One shape falls outside the accelerated set today, and it is worth naming because neural-network code runs into it. **`linalg:erf`** is an `emap` over a scalar series, and `emap` is never intercepted, so the exact [`torch:gelu`](../reference/functions/torch-gelu.md) gains nothing from the flag (its `:approximate :tanh` form is `mul` / `add` / `tanh` and is accelerated). That is a gap to be closed rather than a deliberate exclusion -- unlike `emap` with a user callback, `det` / `inv`, and `array-equal`, which are excluded by design.
