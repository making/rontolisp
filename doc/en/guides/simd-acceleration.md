...
**The rest of the element-wise set is refused, and refused by measurement.** `sqrt`, `abs`, `negative`, `sign` and the binary `add`, `sub`, `mul`, `div` stay on the CPU at every size. They are one machine instruction per element, so their cost on the CPU is already just the cost of walking the array -- and a device has to walk it twice, over a link slower than memory, before it can start. Measured over 1.5 M elements, `linalg:sqrt` is 700 us on the CPU against 502 on the device (and 500 against 245 at single width, the flag's best case for it), while `linalg:add` is 900 us against 780 -- and at single width the CPU **wins**, 350 us against 382. A member that wins by less than the measurement's own noise is not a member.
...
End to end, `examples/llm-from-scratch/chapter03/train-gpt-soseki.lisp` at the notebook's own shapes (`*n-embd*` 384, `*block-size*` 256, which the file says is a one-line change) runs a training step **2.3x faster on the JVM class output** with the flag on -- 0.82 s to 0.36 s, from a 5-step and a 20-step run so that setup and sampling fall out of the slope. Two thirds of that is the stacked matrix product and the last third is the transcendentals this section added; what remains on the CPU is `softmax`, `layer-norm` and the optimizer update, each of which is a CHAIN of the cheap element-wise members the flag refuses one at a time. On the **interpreter** the same program still shows no change at all -- 26.1 s per step against 25.5 -- and the reason is not the device: an interpreted step is 32 times a compiled one at the same shapes, so what dominates it is the tree walk around the kernels rather than the kernels. Compile the program before you measure a flag.
