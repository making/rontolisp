...
- Expect a few times the serial kernel on a large product, not the core count: a matrix-by-vector product streams its matrix once, so memory bandwidth is the ceiling. On a 20-core box a 32000x288 single-float matrix-by-vector product gains about 3.8x, a 768x288 one about 1.8x, and the `examples/llama2` decode loop about 1.9x over `--simd` ([its README](../../../examples/llama2/README.md)).
