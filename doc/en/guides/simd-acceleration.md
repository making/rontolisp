...
- **Reductions along an axis are bit-identical.** `(linalg:sum a 0)`, `(linalg:amax a 1)` and the other axis forms fold exactly as the portable definitions do -- in double, in the same order, with the same tie rules -- so unlike the whole-array reductions they cannot differ.
