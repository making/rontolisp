...
| `flexi-streams` | pass-through streams (a flexi stream IS the underlying stream), plus the REAL in-memory octet pair: `make-in-memory-input-stream` over the `vector-stream` class, and `make-in-memory-output-stream` / `get-output-stream-sequence` (which resets the stream, as upstream does). See [Gray Streams](gray-streams.md) |
