...
| [mito](https://github.com/fukamachi/mito) 0.2.0 (`mito-core` only) | interpreter, JVM, WASM component | O/R mapper core from its verbatim sources via `(ql:quickload '("mito-core" "dbd-postgres"))` (the `mito.core` package; the full `mito` package is the migration layer, not yet loadable): `mito.core:connect-toplevel`/`disconnect-toplevel`, `deftable` (the `dao-table-class` metaclass over the static metaobject protocol — auto-pk `:serial`, `record-timestamps-mixin`'s `created-at`/`updated-at`, `:conc-name` accessors), `ensure-table-exists`, `insert-dao`/`save-dao`/`delete-dao`, `find-dao`, `select-dao` with sxql clauses, `object-id`, `retrieve-by-sql` and `execute-sql`. PostgreSQL only (`dbd-postgres`); like every metaclass consumer, `deftable`/`defclass` forms must be top level with literal options. The uuid dependency loads (its v1/v4 generation draws the backend's own entropy — `make-random-state` answers `nil` and `random` ignores the state argument), and `dissect`'s stack introspection is the no-op interface (no backend carries a Lisp call stack). Same socket constraints as cl-dbi: Preview 1 WASM is out, a component needs `-W exceptions=y -S tcp=y -S inherit-network=y` |
