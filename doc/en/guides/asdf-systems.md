...
| [assoc-utils](https://github.com/fukamachi/assoc-utils) | all four | Eitaro Fukamachi's alist utilities — `aget` (with a default, and a settable `setf` place), `alist-keys`/`alist-values`, `alist-plist`/`plist-alist`, `remove-from-alist` and its `delete-from-alistf` place variant, `alist-hash`/`hash-alist`, `with-keys`, the key-path `alist-get` and `alist=` | **`alistp` is unreliable on a non-alist on the compile paths** — its early exit out of a `mapl` lambda is a lambda-local return there, so a compiled `alistp` can report `t` for a value a real ASDF host would reject (the interpreter is correct) |
