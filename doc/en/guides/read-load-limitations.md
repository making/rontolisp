...
The reader-level syntax that the frontend resolves at read time — backquote templates, `#| ... |#` block comments, `#+`/`#-` feature conditionals, and the `*features*` substitution (see [Data Types](../reference/data-types.md#comments-feature-conditionals-and-features)) — is **not** understood by the runtime reader of compiled output: a file read at runtime via `read`/`read-from-string`/a computed `load` must not use them.
