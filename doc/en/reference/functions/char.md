...
Returns the character at the 0-based `index` of `string`. `char` and `schar` behave identically here; in Common Lisp `schar` is the simple-string variant, but rontolisp treats them the same. The WASM backend indexes strings by byte, so indexing is well-defined for ASCII text only. A `string` that is not a string or an `index` that is not an integer signals a `type-error` naming the operator (`CHAR: The value 5 is not of type STRING`); the `setf` forms below report as `(SETF CHAR)` / `(SETF SCHAR)`, and so does a stored value that is not a character (`(SETF CHAR): The value 5 is not of type CHARACTER`).
