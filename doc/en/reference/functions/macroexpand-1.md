...
Deviations from Common Lisp: rontolisp has no multiple values, so there is no second `expanded-p` value — compare the result with the input to detect expansion — and no environment argument. On the compilation path the argument must be a literal quoted form: the CLI folds the call to its expansion at compile time (the macro table does not exist at runtime in compiled output), so a computed argument is a compile error, and the runtime `eval` of a compiled program does not know `macroexpand-1`.
