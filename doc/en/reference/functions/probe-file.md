...
The "truename" answered on success is a pathname carrying the argument namestring: no backend resolves symbolic links or makes the path absolute. The argument takes either spelling (a pathname or a namestring). The path is interpreted exactly as `open` interprets it -- relative to the process working directory on the interpreter and the JVM, and to the first preopened directory on WASM (run with `--dir`). A directory counts as existing. `uiop:file-exists-p` is the same operation under its ASDF/UIOP name.
