...
Answers whether a file exists: the pathname when it does, `nil` when it does not. This is the only file operation that asks the question without opening anything, and the only one that does not fail on a missing path -- `open` (and therefore `with-open-file`) signals an error on the interpreter and the JVM and *traps* on WASM, and a WASM trap cannot be caught by `handler-case`, so wrapping an `open` is not a portable substitute. Works on all four backends.
