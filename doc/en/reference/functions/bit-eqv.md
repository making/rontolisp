...
Element-wise equivalence (not-xor) of two bit arrays of the same dimensions, answered as a bit vector. Both inputs must be bit arrays -- a `#*` literal or a [`make-array`](make-array.md) result with `:element-type 'bit`, of any rank -- with equal dimensions; anything else signals an error.
