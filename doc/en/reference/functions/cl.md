...
| `file-position` | `(file-position s)` | the position of a file stream (elements of a binary one, bytes of a character one), or the reposition when a second argument is given; `nil` for any stream whose position cannot be determined |
