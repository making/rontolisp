...
Fills the gaps in `pathname` from `defaults` and returns the merged namestring.
A pathname is its namestring in rontolisp, so the rule works on the two parts a
namestring has: the *directory* (everything through the last `/`) and the
