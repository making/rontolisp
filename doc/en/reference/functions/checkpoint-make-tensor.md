...
A packed float array of `shape` -- a dimension list, or one integer for a vector -- and `element-type`, `single-float` or `double-float`, filled with zeros. Unlike a bare `make-array`, it **verifies** that the array it got is packed: `make-array :element-type` answers a boxed general array for an element type it does not recognise instead of signalling, and a checkpoint read into one would only show as slow, wrong output much later. This is the one allocation path the readers use.
