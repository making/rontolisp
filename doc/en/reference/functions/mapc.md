...
Each argument must be a list (`nil`, the empty list, is accepted); passing a non-list such as a string signals a `type-error` rather than silently doing nothing. Use `map` to map over a string or vector.
