...
Writes the elements of `sequence` -- a one-dimensional array of integers between 0 and 255 -- to a binary output stream and returns the sequence. Writing starts at index `:start` (default 0) and stops before index `:end` (default the array length). The `:start`/`:end` keywords must be literal; their values may be arbitrary expressions. Works in all three backends; it expands into a `write-byte` loop, so it requires a stream opened with `:direction :output :element-type '(unsigned-byte 8)`.
