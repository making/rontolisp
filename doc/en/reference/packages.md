...
- **`torch`** — a PyTorch-style tensor with reverse-mode automatic differentiation (`torch:tensor`, `torch:matmul`, `torch:backward`, ...) over the `linalg` kernels, implemented once in Lisp source and available in every backend. It does **not** use `cl`. See the [Neural Networks guide](../guides/neural-networks.md).
