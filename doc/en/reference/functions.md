...
| `unread-char` | `(unread-char c s)` | Push the character just read back, so the next read returns it again; answers `nil`. [Gray stream](../guides/gray-streams.md) instances only -- a stream handle has no pushback on any backend and signals |
