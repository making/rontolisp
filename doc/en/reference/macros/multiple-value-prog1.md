...
Evaluates `first-form`, then the remaining forms for effect, and returns **all** the values of `first-form` -- [`prog1`](prog1.md) widened to multiple values. The values are captured with [`multiple-value-list`](multiple-value-list.md) and republished with [`values-list`](values-list.md), so they survive whatever the intervening forms do.
