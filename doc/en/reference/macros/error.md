...
The interpreter and the JVM backend (a compiled `.class`, `.jar` or `.war`) add, under that line, the same lines saying where it happened: `at FILE:LINE in FUNCTION` names the innermost form of the program's files the condition passed through and the program's named function that form is written in (a form in a lambda belongs to the function the lambda is written in, whatever calls it; a library function is never the one named), and a condition that escaped an [`rontolisp:async-defun`](../special-forms/rontolisp-async-defun.md) body adds one `in NAME (async), awaited at FILE:LINE` line per `await` that rethrew it. The report line itself never changes, so what `handler-case` and `princ` see is unaffected. A program given with `-e` prints the report line alone, and so does a wasm-GC build unless it was compiled with [`--report-locations`](../../compiling/wasm.md#uncaught-condition-locations---report-locations).
