...
Supported on every backend except `--no-gc`, which keeps the historical lowering to the primary form. A restart-system program compiles in EH mode on the wasm-GC backends. Lite deviations: `&optional` clause parameters take `nil` instead of their default when not supplied, restarts are not associated with conditions (the optional condition argument of `find-restart` is ignored), and a restart object prints as a plain list rather than `#<RESTART ...>`.
