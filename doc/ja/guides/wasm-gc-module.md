...
乱数だけは選択の余地があるサービスです: モジュールは自前の生成器を持ち(シードを与えなければどのインスタンスも同じ列を生成します)、ホストはエクスポートされた
`__ronto_seed_random` でシードを与えられ、`--host-random` は `random` をホストのインポートに向けます —
`rontolisp:random-bytes` がここで動くようになるのもこれです。[乱数ガイド](random.md)を参照してください。
