...
`:world` / `:bodies`（各ソリッド自身の座標系。運動連鎖を読み取れるようにするもので
す）/ `:both` / `nil` を選びます。`scene:window-of` と `scene:context-of` は
...
れません。描画は上に書いた `scene` の担当で、それはこのパッケージの一部ではなく利用
者です。STL ファイル由来の面を持つ立体は、単なる `geom:polyhedron` です。
