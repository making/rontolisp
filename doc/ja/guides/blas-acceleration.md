...
このフラグが、それが存在する理由であるプログラムに届くようになるのは `vec` 側のおかげです。1 トークンずつデコードする Transformer は、すべての重み行列を別の行列ではなく*ベクトル*に掛けます。したがって `examples/ml/simd-gemv.lisp`・`examples/ml/tiny-llm.lisp`・`examples/llm/llm.lisp` は端から端まで GEMV です。**`--blas` 単独で十分です** — `vec` に届かせるために `--simd` を併記する必要はありません。
...
**それが問題になるかどうかはライブラリ次第なので、上限を設ける前に測ってください。**Apple のマシンでは、Accelerate はデコードループが出す大きさの積を既に 1 スレッドで実行しており、上限を設けても大きな積が欲しがるスレッドを取り上げるだけです。同じ llama2 stories15M のデコードで `--simd` が 538 tok/s、`--simd --blas` が **1021〜1028 tok/s**、`VECLIB_MAXIMUM_THREADS=1` を付けると 920〜943 tok/s でした。64 コアの Linux マシンに OpenBLAS という組み合わせでは同じ 3 つが逆を向き、そちらでは上限を設けるのは礼儀の問題ではなく、勝ちと惨敗を分けます。

**マルチスレッドのライブラリ相手の GEMV のループでは、上限を設けるのは礼儀の問題ではありません。勝ちと惨敗を分けます。**行列×ベクトルの積はメモリ律速で短いため、マルチスレッドのライブラリは呼び出しごとにスレッドバリアの代償を払い、その呼び出し自身ではそれを償却できません。64 コアのマシンで `examples/llm/llm.lisp` に stories15M をデコードさせると、JVM バックエンドで `--simd` が 102〜110 tok/s、`OPENBLAS_NUM_THREADS=1` を付けた `--simd --blas` が **124 tok/s**、ライブラリの既定スレッド数のままの `--simd --blas` は **16 tok/s** でした。rontolisp はこの変数を代わりに設定しません — ライブラリのスレッドプールの大きさを決めるのは利用者です — ので、自分で設定してください。
