...
backquote テンプレートと `*features*` の置換（[データ型](../reference/data-types.md#comments-feature-conditionals-and-features)を参照）はフロントエンドの read 時構文で、コンパイル出力のランタイムリーダーは解決しません。`read`/`read-from-string`/計算された `load` で実行時に読み込まれるファイルでそれらを使うことはできません（`#| ... |#` ブロックコメントはスキップ**されます**。`#+`/`#-` は前述の通りシグナルします）。
