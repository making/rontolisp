...
オクテットはオクテットのままです。バッファ済みの `:raw-body` 自体がバイト
ストリームなので、`(unsigned-byte 8)` のベクタとして届いたチャンクはそのまま
渡ります。バイナリのアップロードはバイト単位で正確であり、デコードされるのは
テキストを渡すソースだけです。
...
そこへドレインされ、オクテットのボディは 1 オクテット 1 文字で書き出されます
(JSON 文字列にとってはそれが唯一の綴りです)。バイナリのレスポンスがバイト単位で
正確にならないのはここだけであり、シンクが存在する理由もそこにあります:
バイナリを返すホストはシンクを渡してください。
