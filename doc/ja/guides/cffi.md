...
> **現時点ではインタプリタのみ。** `cffi` は `java -jar rontolisp.jar` と REPL で
> 動きます。どちらの WASM バックエンドにも foreign function API はないので、
> そうしたプログラムを `.wasm` にコンパイルすると `Cannot compile: FFI:...` エラーに
> なります（恒久的な仕様です）。ネイティブバイナリと `-o Prog.class` 出力は、まだ
> このバインディングを持ち運びません。
