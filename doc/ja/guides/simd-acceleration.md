...
- [**GPU アクセラレーション(`--gpu`)**](gpu-acceleration.md) — その行列積、要素ごとの超越関数、そしてブロードキャスト・軸方向の畳み込み・軸リストの転置の形を NVIDIA のデバイスへ。
