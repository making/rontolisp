...
ひとつだけ知っておく価値のある形があります。これは JVM ではなく我々の側の事情です。`#f` (`single-float`) のリダクションをスカラー基準とビット単位で一致させるため、`vec:dot` / `vec:sum` / `vec:matvec` は各 `f32` レーンを `f64` に拡張してから累算します。この拡張はクリティカルパス上の追加のベクトル演算であり、JVM のイントリンシックから最も抜けやすい演算でもあります。単精度のリダクションで `--simd` の方が遅いと計測されたら、まずここを疑ってください。同じリダクションでも `#d` (`double-float`) なら拡張は含まれず、要素ごとの `#f` カーネル(`vec:add` など)にも含まれません。

JVM バックエンドが読みにくい理由はもうひとつあり、そちらは SIMD とは無関係です。コンパイルされた Lisp の数値ループは中間値をすべてボックス化します。配列要素の読み出しごと、積ごと、累算ごとに `Double` が 1 個、さらにループカウンタごとに `Long` が 1 個。つまりスカラーの `vec:` カーネルの律速は演算ではなく確保とディスパッチです。そのボックス化をどれだけ消去できるか(エスケープ解析とインライン化による)は JVM によって大きく異なり、スカラー経路の方が加速版より速くなる JVM もあります。`--simd` はこの問いを迂回します。カーネルを、そもそもボックス化しないプリミティブな `double[]` / `float[]` のループに置き換えるからです。
...
単精度のリダクションにはもう 1 つ注意があります。WASM の `--simd` カーネルは `#f` のリダクションを単精度のまま累算し、最後の結果だけを拡張します。一方インタプリタと JVM のカーネルは、スカラー基準と同様に各要素を拡張して double で累算します。したがって、単精度の累算器では保持しきれないデータに対する `#f` の `vec:dot` / `vec:sum` / `vec:matvec` は、これらのバックエンド間で最終 ULP を超えて — おおよそ単精度のイプシロン程度 — 食い違うことがあります。単精度のリダクションをバックエンド間で最終ビットまで一致させる必要があるなら、`#d` を使ってください。
...
浮動小数点数ではなく整数の `argmax` の添字を表示するので、アクセラレーションの有無で出力は同一になります。変わるのは経過時間だけです。Apple M4 では wasm-GC が 467 ms -> 3.9 ms、インタプリタが 4.67 秒 -> 0.68 秒でした。
