...
エンドツーエンドでは、`examples/llm-from-scratch/chapter03/train-gpt-soseki.lisp` をノートブック本来の形状(`*n-embd*` 384、`*block-size*` 256。ファイル自身が 1 行の変更だと書いています)で動かすと、**JVM のクラス出力での学習 1 ステップがフラグありで 1.9 倍**速くなります(0.79 秒 → 0.43 秒)。26 倍ではありません。その理由は知っておく価値があります。スタックされた積がデバイスに載ったことでステップの約半分は消え、残り — 要素ごとの層、softmax、layer norm、厳密な `gelu` — はまだすべて CPU にあるからです。**インタプリタ**では同じプログラムがまったく変わりません。理由は同じもののより極端な形です。この形状での厳密な `gelu` は 150 万要素に対する [`linalg:erf`](../reference/functions/linalg-erf.md) であり、`--simd` が加速するようになった今でもその 1 回の呼び出しに 0.16 秒かかります。デバイスが引き受けた行列積は 0.007 秒です。加速対象に入る前は 20 秒かかっており、それを加速したことでインタプリタでの実行時間は半分になりました(5 ステップで 5 分 32 秒 → 2 分 52 秒。`--gpu` の有無によらず同じです)。行列積のアクセラレーションがプログラムを動かすのは、そのプログラムが行列積に時間を使っているときだけです。
