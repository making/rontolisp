...
**その 1 ステップに残っているものはもう `linalg` ではありません。**これは別のフラグに手を伸ばす前に知っておく価値があります。加速後の 1 ステップを実行プロファイルで見ると、およそ 3 分の 1 が AdamW のパラメータ更新に、さらに 7 分の 1 がドロップアウトの乱数生成に費やされています。どちらも `torch:` の中に Lisp で書かれた 1 要素ずつのループで、どのアクセラレーションの継ぎ目にも載っていません。この節が扱っている `linalg` のカーネルは全体の 10 分の 1 程度です。**インタプリタ**では同じプログラムが相変わらずまったく変わりません(1 ステップ 26.1 秒に対し 25.5 秒)。理由はデバイスではありません。同じ形状でインタプリタの 1 ステップはコンパイル済みの 32 倍かかっており、支配しているのはカーネルではなくその周りの構文木の歩行だからです。フラグを測る前にプログラムをコンパイルしてください。
