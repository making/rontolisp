...
現時点で加速対象から外れている形が 2 つあり、どちらもニューラルネットワークのコードが必ず踏むため明記しておきます。**バッチ行列積** — どちらかの被演算子がランク 3 以上の `linalg:matmul` であり、`torch.bmm` すなわち attention 層そのものです — は移植可能な定義を実行します。フラグを付けても速くならず、wasm-GC ではわずかに*遅く*なります。`--simd` 下ではパックド配列がレーングループのブロックになり、個々の要素へのアクセスが高くつくためです。**`linalg:erf`** はスカラー級数を `emap` したもので、`emap` は決して intercept されないため、厳密な [`torch:gelu`](../reference/functions/torch-gelu.md) も何も得られません(`:approximate :tanh` 形は `mul` / `add` / `tanh` だけなので加速されます)。この 2 つは意図的な除外ではなく、塞ぐべき穴です。ユーザーコールバックを取る `emap`・`det` / `inv`・`array-equal` が設計上の除外であるのとは異なります。
