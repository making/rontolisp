...
`--gpu` が届くのは**インタプリタ**(ネイティブバイナリを含む)です。CUDA ドライバは foreign function API 経由で呼ばれ、WASM にはそれがないので、`.wasm` 出力に対する `--gpu` は黙って何もしないのではなくエラーになります。JVM のクラス出力はまだ対応しておらず、同じ理由でエラーです。コンパイルされたプログラムには `--simd` があり、`.class` にはさらに `--blas` があります。以下の数値は `java -jar` のものです。ネイティブバイナリでは現状デバイス呼び出し 1 回のコストが数倍かかり(n=512 の倍精度の積 1 回で JVM の 0.7 ms に対し 18.5 ms)、そのビルドでは測定したすべてのサイズで `--gpu --blas` が `--blas` 単独より遅くなります。それでもそこで `--gpu` は `--simd` の 2 倍以上、移植版の定義の 4 桁速いままです。
