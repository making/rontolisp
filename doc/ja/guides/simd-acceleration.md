...
パックド浮動小数点配列は、ほとんどのターゲットでは普通の GC 対象の値ですが、v128 命令を出力する 2 つのターゲットでは WebAssembly の線形メモリ上のブロックになります。WebAssembly の SIMD は線形メモリしか参照できないためです。メモリの増加を意識する必要があるのはこの 2 つだけです。
...
| wasm-GC `--simd`(`-o prog.wasm --simd`) | 線形メモリ(bump 割り当て) | **いいえ — 一切解放されないため、メモリ増加を意識する必要があります** |
| `--no-gc`(`-o prog.wasm --no-gc`) | 線形メモリ(bump 割り当て) | **いいえ — 一切解放されないため、メモリ増加を意識する必要があります** |

GC のあるターゲットでは、中間結果を捨てるループのメモリ使用量は横ばいのままです。wasm-GC(`wasmtime run -W gc`)で 1024 要素のベクトルを 200000 回新規に作っても、ピークは 50000 回のときと同じ約 123 MB です。アロケータを通過した総量は 1.5 GB あるにもかかわらず、です。`linalg` の配列も同じパックド型なので、まったく同じように振る舞います。

SIMD を出力する 2 つの WASM ターゲットは設計上これと異なります。bump アロケータに解放はないので、**ベクトルを返すカーネルはすべて、アリーナ全体が捨てられるまで恒久的にメモリを消費します**。

`--no-gc` ではアリーナはエクスポート呼び出しの境界で捨てられます。`--no-gc` モジュールには必ずその境界があります(トップレベルに置けるのは `defun` と `rontolisp:wasm-export` 指示子だけで、`_start` は存在しません)。
...
wasm-GC `--simd` にはそのような境界がありません。プログラムは普通の `_start` なので、パックドアリーナは実行中ずっと伸びる一方です。モジュール内のそれ以外(コンスセル・文字列・一般配列)は GC ヒープに残り、これまでどおり回収されます。移動するのはパックド浮動小数点配列だけです。

これこそが、次節の書き込み先を渡すカーネルの存在理由です。`--no-gc` の文字列(`concatenate`・`subseq`・`princ-to-string`)も同じように bump 割り当てされます。`linalg` の配列も同じパックド型なので、wasm-GC `--simd` ではアリーナへ移動します(`linalg` は `--no-gc` ではそもそもコンパイルできません)。

`--simd` なしの wasm-GC の線形メモリにあるのは、インターン済みシンボル名と文字列ストリームのバッファだけです。実行時文字列は「線形メモリが伸び続けるのをやめさせるため」にこそ GC ヒープへ移されました。
...
これこそが、線形メモリを使う 2 つのターゲットを実用的な数値計算ループで使えるものにします(上のメモリの表を参照)。`-into` を使えばピークメモリは実際に生かしているベクトル分だけになります。`--no-gc --simd` での実測では、65536 要素のベクトルを 12000 回累算したときのピーク RSS は `vec:add-into` で 13.7 MB、`vec:add` では 4.31 GB(その後トラップ)でした。wasm-GC `--simd` では、65536 要素のベクトルを 2000 回累算したときのピーク RSS は `vec:add-into` で 22 MB、`vec:add` では 1.16 GB でした。GC のあるターゲットでは `-into` は正しさに何の影響もありません。そこでは単なる確保量の最適化です。
...
- **wasm-GC `--simd` ネイティブ `v128`**: `rontolisp prog.lisp -o prog.wasm --simd` は `vec:` カーネルを WebAssembly 固定幅 SIMD(`f64x2.*`、単精度では `f32x4.*`)にロワリングします。これらの命令は線形メモリしか参照できないため、`--simd` はパックド浮動小数点配列を(そしてそれだけを)GC ヒープから bump 割り当ての線形アリーナへ移します。上のメモリの表を参照し、ホットループでは `-into` 版を優先してください。それ以外はすべて GC 対象のままで、`vec:` API 全体(`vec:matvec` や `vec:from-list` / `vec:to-list` を含む)はそのまま動き、結果も変わりません。`--component` や `--optimize` とも併用できます。実行はこれまでどおり `wasmtime run -W gc` です(wasmtime は SIMD 提案を既定で有効にしています)。
...
wasm-GC で速度差が大きいのは、`--simd` が 2 つのものを同時に置き換えるからです。ボックス化の多いスカラー `vec.lisp` の defun と、1 要素ずつ回すループの両方です。8192 要素のベクトルに対する `vec:dot` を 20000 回反復すると、`wasmtime run -W gc` でスカラーは約 10.2 秒、`--simd` では約 0.03 秒です。
