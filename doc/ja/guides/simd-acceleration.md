...
`vec` パッケージは、移植可能なパックド `f64` ベクトルカーネルを提供します。[パックド浮動小数点配列型](../reference/data-types.md)に対する、コンストラクタ・要素アクセス・要素ごとの算術・リダクションです。double のベクトルに対する密な数値ループのための第一候補のパッケージであり、4 つのバックエンドのうち 3 つでは任意のハードウェアアクセラレーション(SIMD)層を備えます。パッケージ名は移植可能な抽象を表し、`--simd` フラグはそれをどうアクセラレートするかを表します。
...
> **この表は将来変わります。** WebAssembly の SIMD 命令は線形メモリしか参照できないため、wasm-GC バックエンドに `--simd` を尊重させるにはパックド配列を GC ヒープから線形メモリへ移す必要があります。それが入ると wasm-GC の行は 2 つに分かれ、その `--simd` 側は `--no-gc` と同じ「線形メモリ・回収されない」になります。
