...
- 大きな積で逐次カーネルの数倍を期待してください。コア数倍にはなりません。行列×ベクトル積は行列を一度流すだけなので、メモリ帯域が上限です。20 コアのマシンでは、32000x288 の単精度行列×ベクトル積が約 3.8 倍、768x288 のものが約 1.8 倍、`examples/llama2` のデコードループが `--simd` 比で約 1.4 倍です([その README](../../../examples/llama2/README.md))。
