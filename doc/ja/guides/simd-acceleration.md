...
| 幅の混在(`#d` と `#f`) | 許容 — 両方を広げ、第 1 引数の幅が勝つ | 許容 — 規則は同じ。ただし加速カーネルはこの組を受け取らないので、`--simd` でもスカラー定義が走る |
...
経験則: **デフォルトでは `linalg` に対して書いてください。** より広い numpy 流の API で、混在した入力も許容し、`--simd` を付ければ同じカーネルで加速されます。`vec` に手を伸ばすのは、その 2 つの専売特許が目的そのものであるときです: アロケーションのないホットループ(`-into` カーネル)と `--no-gc` ターゲット — 後者は幅の間違いが今もエラーになる唯一の場所でもあり、しかもコンパイル時エラーです(`--no-gc` はパックド幅を型として区別します)。
...
3 つめの幅も同じカーネルに乗ります。**bfloat16**(`#bf16(...)` / `:element-type 'bfloat16`)は 1 要素 16 ビット -- `#d` の 1/4 -- で、WASM バックエンドに bfloat16 配列が存在しないため、インタプリタと JVM のみで使えます([パックド浮動小数点配列](../reference/data-types.md))。移植版の定義は他の 2 つとまったく同じようにこの幅を保ち、畳み込みます。`--simd` がこの幅に対して行うことはもっと狭く、独立した節があります: [`--simd` における bfloat16 幅](#the-bfloat16-width-under---simd)。
...
要素ごとの単項、numpy の ufunc 名で(新しいベクトル): `vec:exp`・`vec:log`・`vec:tanh`・`vec:sin`・`vec:cos`・`vec:tan`・`vec:asin`・`vec:acos`・`vec:atan`・`vec:sinh`・`vec:cosh`・`vec:sqrt`・`vec:abs`・`vec:square`・`vec:negative`・`vec:sign`・`vec:reciprocal`(`1 / x`)。それぞれ各バックエンド自身のスカラー演算を要素ごとに適用します。そのため超越関数のメンバー(`vec:exp` / `vec:log` / `vec:tanh` / `vec:sin` / `vec:cos` / `vec:tan` / `vec:asin` / `vec:acos` / `vec:atan` / `vec:sinh` / `vec:cosh`)はどのバックエンドでも fdlibm の値で(スカラーの `(exp x)` と同じビットです。[数学関数のバックエンド](math-backends.md)を参照)、`vec:abs` / `vec:negative` / `vec:sign` / `vec:tanh` / `vec:sin` / `vec:tan` の `-0.0` の端値は各バックエンド自身のスカラー演算に従います（これらはすべて奇関数または符号を保つ演算なので、結果としてどのバックエンドでも `-0.0` を入れれば `-0.0` が返ります）。`--no-gc` でも超越関数のメンバーは同じ fdlibm 関数を呼び、`vec:sign` は他の WASM バックエンドと同じ符号の命令列で動くので、17 個すべてがどこでも動きます。`vec:sqrt` だけはスカラー演算がバックエンドの `sqrt` と異なります。負の要素は `NaN` を返します（numpy の `np.sqrt`）。CL の `sqrt` は負数を複素平面に写しますが、複素数は packed 要素ストアに格納できないためです。スカラーの `(sqrt x)` 自体は変更ありません。
...
`out` は入力以上の長さが必要です(長さは検査されません。`vec:add` が被演算子の長さを検査しないのと同じです)。要素型は入力と共有していなくても構いません — 幅の違う書き込み先も、他と同じ幅を広げる `aref` を通して書かれます — が、すべての被演算子を 1 つの幅に揃えておくことが、`--simd` で加速カーネルに乗り続ける条件です。
...
## `--simd` における bfloat16 幅

`#bf16` は*格納用*の幅であり、`--simd` もそのように扱います。チェックポイントの重みが実際に読まれる形だけをアクセラレートし、それ以外は移植版の `vec.lisp` の定義に任せます -- 答えは同じで、速くならないだけです。対象は 2 つの形だけで、インタプリタと JVM に限られます(WASM バックエンドにはそもそも bfloat16 配列がありません)。

| 呼び出し | アクセラレート |
|---|---|
| `#bf16` ベクトルに対する `vec:sum` | する |
| `#bf16` ベクトルと `#f` ベクトルの `vec:dot` | する |
| `#bf16` 行列 x `#f` ベクトルの `vec:matvec` / `vec:matvec-into` | する |
| 2 つの `#bf16` ベクトルに対する要素ごとの `vec:add` / `vec:sub` / `vec:mul` / `vec:div`(`vec:+` / `vec:-` / `vec:*` / `vec:/` を含む)、`vec:sqrt` / `vec:abs` / `vec:negative` / `vec:reciprocal`とその `-into` 兄弟 | する |
| `#bf16` オペランドを含むその他すべて | 移植版の定義 |

要はこの行列ベクトル積です。重み行列に対する GEMV はデコードループが時間を費やす場所であり、この幅では転送するバイト数が半分になります。3 つのカーネルはいずれも格納されたパターンをレーンループの*内側*で拡張し(デコードはシフト 1 回)、それ以外は単精度カーネルそのものです。したがってその答えは、拡張したオペランドに対する単精度カーネルの答えと**ビット単位で一致**します。bfloat16 パターンの拡張は厳密なので、この幅が独自の精度の話を持ち込むことはありません。上の[単精度リダクションの規則](#hardware-acceleration-optional)がすべてであり、`vec:matvec` の結果が `x` の幅に従う点も移植版の定義と同じです。

要素ごとの行も、小さな規模での同じ考えです。両方のオペランドをレーンループの内側で拡張し、単精度で計算して、格納時に狭めます -- 狭い幅で中間値を保持することはありません。狭める処理は分岐なしのレーン形式で(複合カーネルの実測ではスカラー経路の 2.3-2.7 倍)、単精度の中間値は移植版の定義の答えとビット単位で一致します。ミラーされるのは単精度のレーンプーループを持つメンバだけです。`vec:scale` は真の倍精度スカラーを掛け、比較セレクトはスカラーループで、超越単項関数は要素ごとに fdlibm（`java.lang.StrictMath`）を呼びます -- いずれもミラーすべきレーン形式を持ちません。

この組み合わせは意図的に狭くしてあります。リダクションは bfloat16 の重みに対する単精度の活性化 -- 公開されたチェックポイントがデコードされるときの組み合わせです -- で、要素ごとのメンバは bfloat16 同士、その結果は移植版の定義と同じくその幅に留まります。それ以外のオペランドの組み合わせはすべて移植版の定義が計算します: 要素ごとの呼び出しでの `#bf16` と `#f`・`#d` の混ざった組み合わせ、幅の広いソースに対する `#bf16` の `-into` 受け皿、この幅での `vec:scale`・比較セレクト・超越単項関数。幅の混ざった要素ごとの呼び出しも拒否されるのではなく計算されます -- `--simd` が答えをエラーに変えることはありません。

融合された GEMV が同じ重みの単精度版より速いかどうかは、重みの大きさ次第です。おおよそ 4 MB 未満では行列がキャッシュに載り、節約できる帯域がないため、デコードのぶんだけわずかに損をします。1 スレッドでの実測は、あるマシンで単精度カーネルの 0.7-0.8 倍、別のマシンではほぼ同等でした。それを超えると半分になったバイト数が速度として現れます -- 4096x4096 ではどちらのマシンでも 1.3-2.0 倍で、[`--parallel`](#using-more-than-one-core---parallel) を付ければさらに上がり、1024x1024 から同等以上になります。サイズによる切り替えはありません(答えが行列の大きさに依存してはならないからです)。小さい行列に `#bf16` を選んだプログラムはそのわずかなぶんを払い、そのかわりどの大きさでもメモリは半分になります。
...
- `RONTOLISP_THREADS` でスレッド数を決めます(呼び出し元のスレッドを含む。デフォルトは利用可能なプロセッサ数の**半分**で、2 を下回ることはありません。`RONTOLISP_THREADS=1` で逐次実行)。全部ではなく半分なのは、行列積が最後のコアに届くよりずっと手前でメモリ帯域に律速されるからです。マシンの残り半分のスレッドはスループットを増やしません。それどころか、マシンと同じ幅のプールは、そのマシンが走らせられるだけのスレッドのうち最も遅い 1 本を毎回の積が待つことを意味します。ほかに実行可能なものが 1 つでもあれば、それが行範囲の途中に割り込み、積はスケジューリング待ちを丸ごと払います。64 スレッドのマシンでは `examples/llm` のデコードループが 64 スレッドで 8.7 トークン/秒、32 で 9.5、16 で 9.6 でした。ワーカースレッドはデーモンスレッドで、分割に値する最初の呼び出しで作られ、それより前には作られません。そうした呼び出しをしないプログラムはスレッドを 1 本も起動しません。呼び出しの合間、ワーカーは眠る前に 1 ミリ秒ほどスピンし続けます -- 1 ミリ秒の別の仕事のあとに来る積でも元が取れるのはそのためです -- ので、積のループが走っている間スレッドはビジーです。[`--blas`](blas-acceleration.md) で `OPENBLAS_NUM_THREADS` を決めるのと同じように、そのマシンの仕事に許される数に合わせてください。1 台のマシンを 2 つのプログラムで分け合うなら、それぞれが欲しいのはマシンの取り分であって、それぞれの半分ではありません。
...
- インタープリタ/REPL では、`jdk.incubator.vector` のない `java -jar` に対する `--simd --parallel` も通常のフォールバックではなくエラーになります。逐次カーネルの `vec.lisp`/`linalg.lisp` の参照実装をスレッドに分割しても、アクセラレーションされたカーネルに対しては高速化ではなく低速化にしかならず、`--parallel` を指定したということはそのプログラムが分割に値するはずだからです。`--add-modules jdk.incubator.vector` を付けて再実行するか、ネイティブバイナリを使ってください。コンパイル済みの `.class` 出力はこれまで通り縮退します -- インタープリタ自身のプロセスと違い、後で別のモジュールを持つ JVM で動く可能性があるからです。
