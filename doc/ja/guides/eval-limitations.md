...
- **`rontolisp` パッケージの関数はサポートされません。** `rontolisp:version`、`rontolisp:list-functions`、`rontolisp:list-macros`、`rontolisp:list-special-forms`、`rontolisp:fetch`、`rontolisp:await` は直接コンパイルされます（定数またはインライン呼び出し）。実行時 `eval`/`load` はこれらを認識しません。
