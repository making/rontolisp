...
- **パラメータの多いユーザー関数は `eval` から呼び出せません。** JVM では 8 個以上のパラメータを持つコンパイル済み関数の呼び出しが `nil` を返し、WASM では 11 個以上のパラメータを持つものの呼び出しが失敗します。
