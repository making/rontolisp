...
| [mito](https://github.com/fukamachi/mito) 0.2.0 (`mito-core` のみ) | インタプリタ、JVM、WASM コンポーネント | O/R マッパーのコアを無改変ソースから `(ql:quickload '("mito-core" "dbd-postgres"))` でロードします (`mito.core` パッケージ。フルの `mito` パッケージはマイグレーション層で、まだロードできません): `mito.core:connect-toplevel`/`disconnect-toplevel`、`deftable` (静的メタオブジェクトプロトコル上の `dao-table-class` メタクラス — auto-pk `:serial`、`record-timestamps-mixin` の `created-at`/`updated-at`、`:conc-name` アクセサ)、`ensure-table-exists`、`insert-dao`/`save-dao`/`delete-dao`、`find-dao`、sxql 句付きの `select-dao`、`object-id`、`retrieve-by-sql`、`execute-sql`。PostgreSQL のみ (`dbd-postgres`)。他のメタクラス利用ライブラリと同様、`deftable`/`defclass` はリテラルオプションのトップレベルフォームである必要があります。依存の uuid もロードされます (v1/v4 の生成はバックエンド自身のエントロピーを使います — `make-random-state` は `nil` を返し、`random` は state 引数を無視します)。`dissect` のスタック内省は no-op インターフェイスです (Lisp のコールスタックを持つバックエンドはありません)。ソケット制約は cl-dbi と同じです: Preview 1 WASM は対象外、コンポーネントは `-W exceptions=y -S tcp=y -S inherit-network=y` が必要です |
