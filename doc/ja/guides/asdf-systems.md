...
| `uiop` | 大部分はパッケージスタブです。実体のあるメンバは [`uiop:getenv`](../reference/functions/getenv.md) (Common Lisp に `getenv` はないため、rontolisp における唯一の綴りです)、`uiop:file-exists-p` ([`probe-file`](../reference/functions/probe-file.md) と同じ操作)、`uiop:merge-pathnames*`、[`uiop:add-package-local-nickname`](../reference/functions/add-package-local-nickname.md) です |
