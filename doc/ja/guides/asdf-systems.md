...
| `flexi-streams` | パススルーのストリーム (flexi ストリームは基底ストリームそのもの) に加え、本物のインメモリ・オクテットストリームの対: `vector-stream` クラス上の `make-in-memory-input-stream` と、`make-in-memory-output-stream` / `get-output-stream-sequence` (upstream と同じくストリームをリセットします)。[Gray Streams](gray-streams.md) を参照 |
