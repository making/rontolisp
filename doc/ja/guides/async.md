...
> ([後述](#under-the-hood-wasi-preview-3-futures--streams))。
> **Preview 1** WASM には非同期のホスト I/O がないため、非同期本体は
...
これはすべて wasmtime 46+ でデフォルト有効なコンポーネントモデルの非同期
...
まで走り、その future は最初から確定済みで生まれます。本体がシグナルしたエラーは
future に保持され、他のバックエンドと同じく `await` のたびに再びシグナルされるので、
`await` を囲む `handler-case` (や `rontolisp:catch`) がそれを受け取れます。ただし
これはコンディションを捕捉しうるプログラムに限った話で、捕捉フォーム
(`handler-case`、`ignore-errors`、`unwind-protect` など) を持たず
`--report-locations` も付けずにビルドしたプログラムは
*呼び出し* の時点で停止し、呼び出しと `await` の間にあるものは実行されません。
`wait-for` / ゲストのストリーム操作はコンパイル時に拒否されます。**`--no-gc`** は
非同期の表面全体を名指しで拒否します。
