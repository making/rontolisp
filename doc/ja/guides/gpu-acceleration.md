...
**さらに 2026-09-02 以降、4 つの合成がそれぞれ 1 パスで実行されます。** `torch:softmax`(`:axis` 形式で、最後の軸に対して)、厳密形の `torch:gelu`、`torch:layer-norm` の正規化、`torch:dropout` のマスクは、順伝播も(最初の 3 つは)逆伝播も、以前は上記のメンバを 5 個から 14 個つないだ鎖としてデバイスに届いていました。各メンバがアクティベーション全体を 1 回なめるパスです。いまは各合成が 1 つのカーネルで、鎖の算術を丸めごとに同じ順序で計算するため、結果はデバイスのメンバの鎖が出していたものとビット単位で同じで、パスだけがなくなります。本の形状で呼び出しごとに測定すると、`(64 256 1536)` に対する `torch:gelu` の順伝播 + 逆伝播が 19.1 ミリ秒から 5.9 に、`torch:layer-norm` のそれが 8.6 から 2.9 に、アテンションヘッド 1 つ分の `torch:softmax` のそれが 1.6 から 0.9 に、dropout のマスクが 2.3 から 1.9 に(こちらも速くなった乱数生成器の充填が、マスクのコストそのものです)。本の形状の学習ステップは時間の 4 分の 1 を失いました。layer-norm、softmax の逆伝播、マスクにはライブラリ関数が入っていないので、移植版の定義と**バイト単位で同一**のままです。softmax の順伝播と GELU は、鎖を通っていたときとまったく同じく、デバイスの `exp` と `erf` を含みます([対象バックエンドと精度](#対象バックエンドと精度))。それ以外の場所ではビットは一切動いていません。どのバックエンドでも、この 4 つは置き換える前の合成そのものを 1 つの呼び出しとして綴ったものです。
