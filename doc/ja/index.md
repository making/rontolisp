...
Java で実装された Common Lisp サブセットです。3 つの実行モードをサポートします。

- **インタプリタ** -- REPL をサポートするツリーウォーク評価。ファイル実行時の
  デフォルトで、スループットよりターンアラウンド向け（ループ中心のコードは
  コンパイルすると 20 倍〜200 倍高速）
...
[エージェントスキル](getting-started/agent-skill.md)（デプロイのたびに再生成）も利用できます。
