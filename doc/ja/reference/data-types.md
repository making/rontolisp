...
|------|---------|-------------|
| Integer | `42`, `-5`, `1,000`, `#xff`, `#o777`, `#b1010` | 64ビット符号付き整数。オーバーフロー時には自動的に多倍長整数へ昇格します(インタプリタおよびJVM)。WASMでは符号付き64ビット範囲で正確で、それを超えるとラップします。`#x`/`#o`/`#b` は16進/8進/2進リテラルとして読み込まれます |
...
`263130836933693530167218012160000000` を返します。**WASMコンパイラ**は、31ビットのfixnum表現からあふれた結果をボックス化された符号付き64ビット値へ昇格させます(そのためMD5のような符号なし32ビットの計算は正確に動きます)が、64ビット範囲を超える演算はラップします。任意精度への昇格はないため、`(fact 32)` が正確なのはインタプリタとJVMだけです。
...
バックエンドごとに、各成分は整数表現に従います。**インタプリタおよびJVMコンパイラ**は多倍長整数を使用し(巨大な分子/分母のratioも正確なまま)、**WASMコンパイラ**はratioの成分をオーバーフロー昇格なしで31ビットのfixnum範囲に保ちます(通常の整数は64ビットへ昇格しますが、割り切れない除算は成分を31ビットへ畳み込みます)。コンパイルされた `read`/`load` のために発行されるランタイムリーダはratioリテラルをパースしません(実行時に読まれる
