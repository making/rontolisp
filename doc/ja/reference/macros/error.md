...
インタプリタと JVM バックエンド(コンパイル済みの `.class`、`.jar`、`.war`)は、その行の下に発生場所を示す同じ行を加えます。`at FILE:LINE in FUNCTION` は、コンディションが通過したプログラムのファイル中で最も内側のフォームと、そのフォームが書かれているプログラムの名前付き関数を示します(ラムダの中のフォームは、誰が呼び出したかにかかわらず、そのラムダが書かれている関数のものです。ライブラリ関数が示されることはありません)。[`rontolisp:async-defun`](../special-forms/rontolisp-async-defun.md) の本体から抜けたコンディションには、それを再送出した `await` ごとに `in NAME (async), awaited at FILE:LINE` の行が 1 行ずつ加わります。報告行そのものは変わらないため、`handler-case` や `princ` が見るものに影響はありません。`-e` で与えたプログラムは報告行だけを出力します。wasm-GC のビルドも、[`--report-locations`](../../compiling/wasm.md#uncaught-condition-locations---report-locations) を付けてコンパイルしない限り同様です。
