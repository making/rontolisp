...
コンパイル経路（`UserMacroExpander`）では JVM/WASM コンパイラが動く前に `macrolet` は完全に展開・除去されます。インタプリタはネイティブに展開します。`macrolet` で標準演算子（関数名前空間の名前）を局所的にシャドウすることはできません。`symbol-macrolet` は未サポートです。
