...
- `~e` は仮数部を整数演算から組み立てるため（どのバックエンドでも出力は同一になります）、桁数はリテラルでなければならず、実行時の `v` は使えません。WASM バックエンドは整数を符号付き 64 ビットの範囲に制限するため、スケーリングされた仮数部により `~,De` の精度はおよそ `D` <= 17 桁に制限されます。デフォルト（`~e`、6桁）はその範囲に十分収まります。`~e` のスケールファクターは 1（デフォルト）のみで、`~f`/`~e` のオーバーフロー文字にはリテラルの幅指定が必要です。
...
- WASM バックエンドでは整数が符号付き 64 ビットの範囲に制限されるため、`~:d` による桁区切り（および `~x`/`~o`/`~b`/`~r`）はその範囲まで正確です。任意精度（多倍長）の値はインタプリタと JVM バックエンドでのみ動作します。
