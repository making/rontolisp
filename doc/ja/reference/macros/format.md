...
- `~e` は仮数部を整数演算から組み立てるため（どのバックエンドでも出力は同一になります）、桁数はリテラルでなければならず、実行時の `v` は使えません。WASM バックエンドは整数を i31 の範囲に制限するため、スケーリングされた仮数部により `~,De` の精度はおよそ `D` ≤ 8 桁に制限されます。デフォルト（`~e`、6桁）はその範囲に十分収まります。`~e` のスケールファクターは 1（デフォルト）のみで、`~f`/`~e` のオーバーフロー文字にはリテラルの幅指定が必要です。
...
- WASM バックエンドでは整数が i31 の範囲に制限されるため、非常に大きい（多倍長）整数の `~:d` による桁区切り（およびそのような値の `~x`/`~o`/`~b`/`~r`）はインタプリタと JVM バックエンドでのみ動作します。
