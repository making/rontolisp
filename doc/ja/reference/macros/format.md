...
その他の出力先（フィルポインタ付き文字列）はサポートされておらず、カラム制御ディレクティブ（`~t`、`~<...~>`）と基数パラメータのない `~r` は実装されていません。ループ脱出の `~^` はトップレベルと `~{ ... ~}` / `~@{ ... ~}` の本体内でサポートされます（結合イディオム `"~{~a~^, ~}"`。`~:{ ... ~}` の中では現在のサブリストの本体を終了します）が、`~:^`/`~@^` とプレフィックスパラメータには対応していません。制御文字列が実行時の値になる場合 -- 計算された制御文字列や、関数値 `#'format` を介した呼び出し（`funcall`/`apply`）-- は、基本ディレクティブ `~a ~s ~d ~x ~c ~%` と `~~` のみをサポートする実行時フォールバックで描画されます（未知のディレクティブはそのまま出力されます）。destination が nil なら文字列を返し、それ以外の destination には 1 回の `write-string` 呼び出しで書き込みます。さらに以下の点に注意してください。
