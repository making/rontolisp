...
`place` を評価し、値が指定された型でなければエラーをシグナルします。型に合致していれば nil を返します。これは Common Lisp の `check-type` のライト版です。`store-value` リスタートを確立しないため、エラーを対話的に修正することはできず、place へ値が再格納されることもありません。
