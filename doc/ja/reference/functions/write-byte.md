...
これは 4 バイト `89 50 4E 47`（PNG シグネチャの先頭）を `data.bin` に書き込みます。インタプリタと JVM では 0-255 の範囲外の値はエラーになります。より広い型や符号付きの整数 `:element-type`（[`open`](open.md) を参照）で開いたストリームでは、1 回の呼び出しで 1 つの要素を、2 の補数のリトルエンディアンのオクテットとして書き込みます。広げた型の範囲外の値はすべてのバックエンドでエラーになります。
