...
rontolisp ではパッケージは読み込み/コンパイル時に解決されるため（[パッケージ](../packages.md)を参照）、リテラルなトップレベル呼び出しは [`in-package`](../special-forms/in-package.md) と同様にコンパイル時に消費され、それ以降のフォームに効果を及ぼします -- これがすべてのバックエンドで動作する理由です。実行時に計算される呼び出し（実行時に組み立てた designator）はインタープリタのみで動作します。内部シンボルは決して継承されません。使用されるパッケージが `:export` したものだけが見えるようになります。
