...
(GC) バックエンドでは着信する整数は `i31ref` で、その範囲を超えると float に
広がるため、`3000000000` という `:u32` 引数は正確な float `3000000000.0` として
Lisp コードに届きます。Lisp コード内の整数演算は符号付き 64 ビットの範囲まで
正確です (`i31` fixnum 範囲を超えた結果はボックス化整数へ昇格するため、`:u32`
引数 `1073741823` に対する `(+ x 1)` は正確に `1073741824` を返します)。宣言した
型で表せない結果だけが境界でトラップします。非 GC バックエンド (`--no-gc`) では
整数を `i64` で計算するので、`:s64`/`:u64` までの族全体が float への拡張なしに
渡ります。
