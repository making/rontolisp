...
ファイルシステムを持つすべてのバックエンド、すなわちインタプリタ、コンパイル済み `.class`/`.jar`、WASM Preview 1、WASI 0.3 コンポーネントで、4 つとも同じ答えを返します。方言の判定が長さではなく形を読むのは、まさにそれを可能にするためです。[`file-length`](file-length.md) は設計上どちらの WASM バックエンドでも `nil` を返します。
