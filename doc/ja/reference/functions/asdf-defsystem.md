...
- `:class :package-inferred-system` — システムは `:components` を一切持たず、グラフはソースから導出されます: サブシステム名がシステムのディレクトリ配下のファイルパスになり (`my-lib/util/text` は `util/text.lisp`。システムが `:pathname` を持つ場合はその配下)、そのファイル先頭の `defpackage` が依存関係を表します。[システムガイド](../../guides/asdf-systems.md#サポート範囲-と非サポート)を参照してください。これ以外の `:class` はサポートしません。
