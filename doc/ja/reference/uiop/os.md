...
で移動します。これは現在すべてのバックエンドがバイナリファイルストリームに対して
サポートしています。それでも 2 つの WASM バックエンドでは `uiop:not-implemented-error`
をシグナルします。実際の `.lnk` に対して本体を動かして確かめていないためで、黙って
誤解析するよりは操作の名前を挙げてシグナルするほうがよいからです。
