...
短形式の [`:method-combination`](defgeneric.md) のもとでは修飾子の集合が変わります。プライマリメソッドは代わりに**結合名**を修飾子として持ち（`(defmethod total + ((x account)) ...)`）、`:around` は従来どおり全体をラップし、`:before`/`:after` は拒否されます。
