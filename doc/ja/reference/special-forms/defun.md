...
必須引数より少ない引数で関数を呼び出すと(固定アリティ関数では多すぎる場合も)、引数を評価したうえで、呼び出しの実行時に捕捉可能な `program-error` を通知します。これはすべてのバックエンドで共通です。コンパイルエラーにはならず、個数の誤った直接呼び出しに対して JVM/WASM のコンパイラは警告を表示します。
