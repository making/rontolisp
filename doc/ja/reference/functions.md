...
| `assoc` | `(assoc 'b '((a . 1) (b . 2)))` | `(b . 2)`(carがキーに一致する最初のペア、またはnil。既定では `eql` で比較し、省略可能な `:test` キーワードに関数指定子を渡せます。例: `(assoc "b" '(("a" . 1) ("b" . 2)) :test #'equal)`) |
...
| `rassoc` | `(rassoc 2 '((a . 1) (b . 2)))` | `(b . 2)`(cdrが値に一致する最初のペア、またはnil。既定では `eql` で比較し、省略可能な `:test` キーワードに関数指定子を渡せます) |
...
| `linalg:array-equal` | `(linalg:array-equal (linalg:eye 2) (linalg:from-list '((1 0) (0 1))))` | `t`(同じ形状かつ数値的に等しい要素。配列自体は `eq` でしか比較できません) |
