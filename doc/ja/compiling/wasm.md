...
wasm-GC のコード生成には、速度と引き換えにバイト数を費やしている箇所が 3 つあり、どれも `--optimize=off` でも `--optimize=default` でも有効です:
...
- `car` と `cdr` — および実行時に解決した関数への `apply` が引数リストを辿る歩み — は、共有リーダーを 1 回呼ぶ代わりに、nil を素通しする読み出しを 1 サイトあたり 17 バイトでインライン展開します。

`--optimize=size` はこの 3 つを断ります。プログラムの計算結果は何も変わりません — 高速経路は残り続けるものの代替として存在していただけです — が、演算は汎用ヘルパ経由になり `car`/`cdr` はすべて呼び出しになるため代償は実在し、その大きさはプログラムがどれだけ整数中心かで決まります(共有リーダーだけならリスト走査ループで 1〜3%):
...
| Cloudflare Worker の `hello-clack` (`--no-wasi`) | 990,451 B | 815,414 B (-17.7%) | — |
...
Preview 1、`--component`、`--native` のいずれでも動作し、デフォルトでは無効です。捕捉フォーム(`handler-case`、`ignore-errors`、`unwind-protect` など)を含まないプログラムは、通常は報告なしのトラップで停止します。このオプションはそうしたプログラムにも `Unhandled condition:` の行を加え、それに伴って捕捉フォームと同じく例外処理プロポーザル(wasmtime 37 以降)を必要とします。ただし標準エラー出力の行き先がない `--no-wasi` では何も加えません。指し示すファイルを持たない `-e` のプログラムには何も加えません。ファイル名はコンパイラが読んだパスで、モジュールに埋め込まれます。このオプションの下でも、プログラム自身の関数への末尾呼び出しは、直接でも関数値を通しても定数スタックで動作します。

ファイルから読んだ関数はどれも、コンディションが抜けていく途中でそれを捕捉します。`line` ではさらに、新しい行で始まるフォームのたびに行を記録します:

| モジュール(`--optimize=size`) | 無効 | `function` | `line` |
| --- | --- | --- | --- |
| `zlib`(chipz による gunzip) | 89,499 B | 94,685 B (+5.8%) | 100,626 B (+12.4%) |
| 3 行の関数 100 個 | 14,092 B | 18,754 B (+33.1%) | 20,765 B (+47.4%) |
| `hello_world`(捕捉フォームなし) | 480 B | 643 B | 654 B |
| `(print (read))`(捕捉フォームなし) | 39,027 B | 42,300 B | 42,313 B |

関数あたりおよそ 45 バイトで、`line` では行ごとにさらに 6〜7 バイトです。捕捉フォームを含まないプログラムでは、これに報告そのものが加わります。プログラムが通知しうるすべてのコンディションの文面で、最後の行が大きく増えているのはこのためです。実行時間は V8(Node 24)では変わりません。wasmtime 49 では呼び出しの多い関数ほど捕捉の代価が大きく、再帰的な `fib` は約 3 倍遅くなります。他のベンチマークプログラムは 0〜15% の範囲に収まります。
