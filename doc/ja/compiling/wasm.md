...
| ライブラリ(リアクター)モジュール | `--no-wasi` | 全機能(純粋計算エクスポート) | インポート不要の任意の wasm-GC エンジン(Node 22+、現行ブラウザ。`--host-random` を付けるとエントロピーのインポートが 1 つ増える) | [`--no-wasi` リアクターモード](../guides/wasm-gc-module.md#no-wasi-reactor-mode) |
