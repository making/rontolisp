...
上記の `fact` の例では、モジュールは約 26 KB から約 1.3 KB に縮小します。`--optimize` はオプトインであり、動作を保存します。実際の `call` 命令からコールグラフを辿るため、到達可能なものはすべて（組み込みの `eval`/`load` がディスパッチするコードを含めて）保持されます。これは WASM 専用であり、`--component` のもとでは **効果がありません**（WASI 0.3 アダプターはコアの固定されたインポート/インデックスレイアウトに依存しています）。JVM のデッドコード除去はまだ実装されていません。
