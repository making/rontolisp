...
| `hello_world`(捕捉フォームなし) | 480 B | 649 B | 660 B |
| `(print (read))`(捕捉フォームなし) | 34,981 B | 42,396 B | 42,404 B |

関数あたりおよそ 40 バイトで、`line` では行ごとにさらに 6〜7 バイトです。捕捉フォームを含まないプログラムでは、これに報告そのものが加わります。プログラムが通知しうるすべてのコンディションの文面で、最後の行が大きく増えているのはこのためです。実行時間は V8(Node 24)では変わりません。wasmtime 49 では呼び出しの多い関数ほど捕捉の代価が大きく、再帰的な `fib` は約 3 倍遅くなります。他のベンチマークプログラムは 0〜15% の範囲に収まります。
