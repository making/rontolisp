...
この `fact` の例では、モジュールは約 330 KB から約 5 KB まで縮みます。`--optimize` はオプトインで、動作を保存します: 実際の `call` 命令から呼び出しグラフを辿るため、到達可能なもの(組み込みの `eval`/`load` がディスパッチするコードを含む)はすべて保持されます。`--component` を含む**すべての**出力形状で有効です — コアモジュールはラップの前にシェイクされます。同じフラグは [JVM 出力](jvm.md)のデッドコード除去も行います。
...
| --- | --- | --- | --- |
| ironclad SHA-256/HMAC/PBKDF2、4096 ラウンド | 2,075,455 B | 1,560,097 B (**-24.8%**) | 1.4 s -> 5.2 s (**3.8 倍**) |
| `vec:` カーネルを使うニューラルネットの学習ループ | 288,576 B | 231,533 B (-19.8%) | 1.07 s -> 1.26 s (+18%) |
| 浮動小数点 MLP の学習ループ(`vec:` なし) | 177,173 B | 142,943 B (-19.3%) | 5.6 s -> 6.1 s (+9%) |
| `cl-postgres` の hello world (`--component`) | 8,033,507 B | 6,408,277 B (-20.2%) | — |
