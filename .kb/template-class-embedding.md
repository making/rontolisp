...
Reach for (3) ONLY when the helper is genuinely too complex/error-prone to hand-assemble AND needs JDK facilities that are impractical in raw bytecode — [[java-interop]] qualifies on both counts (cost-based overload resolution, recursive marshalling, `Proxy` invocation handlers), as do the three acceleration flags' bridges (`jdk.incubator.vector`, a `critical` FFM downcall into a CBLAS, the CUDA driver API); nothing else in the code base currently does.
