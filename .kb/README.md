...
- [standard-output-redirect.md](standard-output-redirect.md) -- the stream-argument-less print family reads `*standard-output*` at call time (the `with-output-to-string (*standard-output*)` capture): the bind-activated special rule, per-backend defaultStreamArg wiring, the fresh-line string-stream/file rule
...
- [error-handling.md](error-handling.md) -- unwind-protect (interpreter try/finally, JVM exception tables + return-escape cleanups), condition objects (define-condition over the CLOS subset, seeded hierarchy, error/signal/warn designators, with-slots), handler-case/ignore-errors (typed catching, ThreadLocal condition/depth channels; only --no-gc rejects catching) and the RESTART system (handler-bind handlers at the signal point + the two global stacks + restart-case over catch/throw, one shared lowering on all four backends)
