...
- [scheme-frontend.md](scheme-frontend.md) -- the EXPERIMENTAL Scheme front end: lowering table, library tags, `--scheme-standard`, destination-driven loops, identifier escaping, hygienic `syntax-rules` macros, `define-library` / `include`, `(scheme char)`'s generated Unicode tables, ports and `(scheme file)` file ports, the `#f` / loop-shape / tail-depth / size measurements
