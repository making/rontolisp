...
- [instance-syntax.md](instance-syntax.md) -- the ONE instance value model behind defstruct/CLOS/conditions: the `%obj-*` primitive seam, `#S(...)`/`#<...>` printing, per-backend shapes (JVM `String[]` discriminator, WASM `TYPE_INSTANCE` + bake-all), the emit gate, and the structural-`equal` decision record
