...
- [scheme-frontend.md](scheme-frontend.md) -- the EXPERIMENTAL Scheme front end: lowering table, library tags, `--scheme-standard`, destination-driven loops, identifier escaping, hygienic `syntax-rules` macros, `(scheme char)`'s generated Unicode tables, the `#f` / loop-shape / tail-depth / size measurements
