...
On by default in the CLI (`--optimize[=LEVEL]`, absent flag = `DEFAULT`, declined with `--optimize=off`); `WasmLispCompiler(dynamic, component, noWasi, optimize)` / `JvmLispCompiler(className, dynamic, optimize)` / `NoGcWasmCompiler(optimize)` all take a `compiler.OptimizeLevel`.

**Invariant: the level-less compiler constructors are `DEFAULT`, not `NONE`** (todo-450, 2026-08-19). `new WasmLispCompiler()`, `new JvmLispCompiler(name)`, `new NoGcWasmCompiler()` and their level-less siblings compile the same shape the flagless CLI does, so there is ONE answer to "what does rontolisp do by default" no matter which frontend asked. The reason this is not merely tidy: the browser playground (`RontoPlayground.compileJvm` / `compileWasm`, `src/web/java`) calls exactly those constructors and has no CLI and no flags, so a `NONE` default there is a default nobody can override -- it would hand a user 156,641 bytes of the hello world the CLI ships in 532. `WasmLispCompiler`'s internal `Ctx.optimize` and its nested builder's field say `DEFAULT` for the same reason (both are overwritten by every constructor and only ever asked `prefersSizeOverSpeed()`, which `NONE` and `DEFAULT` answer alike -- they are inert, and they must not read as a second default). This is a breaking change for an embedder, and the right one: the level is what the artifact IS, and a caller who names none gets what the project's own frontend gives. A test that asserts on the UNOPTIMIZED shape -- a function/import/type is present, a byte-identity pair, a "before" size -- therefore says `OptimizeLevel.NONE` out loud.
...
**Invariant: an ABSENT `--optimize` is `DEFAULT` too, and `NONE` is spelled `--optimize=off`** (todo-448/449, 2026-08-19). The two shapes were ~300x apart on a hello world (156,641 vs 532 bytes of wasm-GC), so the flagless build was a shape nobody ships and the flag was boilerplate pasted into every build script -- the one invocation that forgot it silently shipped the big one. `OptimizeLevel.parse(null)` therefore answers `DEFAULT`, and every level has a spelling. What `off` promises is byte-identity with what a flagless build used to produce: `NONE` still gates the same six `eliminatesDeadCode()` sites and nothing else moved with the flip. Pinned by `OptimizeLevelTest.theAbsentFlagIsTheDefaultLevel` / `theOffSpellingIsNoOptimizationAtAll` and by `JvmLispCompilerTest.theFlaglessBuildIsTheOptimizedOneAndOffIsTheWayBack` (the absent flag compiles the `DEFAULT` bytes AND those differ from `off`'s -- the first assertion alone would also pass if the flip had changed nothing). A near-miss spelling (`none`, `no`) is the same `IllegalArgumentException` as `high`, never a silent `DEFAULT`.

> **Historical rows in this file that say "no flag" mean `--optimize=off`.** They were measured when the absent flag was `NONE`, and they are kept as measured: the pair "no flag vs `--optimize`" is what each one records.
...
| (flag absent), `--optimize`, `--optimize=default` — `DEFAULT` | yes | no |
| `--optimize=size` — `SIZE` | yes | yes |

Those two predicates ARE the level: `OptimizeLevelTest.everyLevelIsDistinguishableAndSpellable` fails if two levels answer both the same way, which is the rule "**do not ship a level that is an alias of another**" made mechanical. It is why there is no `high`: nothing in the compiler is held back for being too aggressive, so a third level would have nothing to switch on, and a synonym would teach a reader that levels are decoration. An unknown value is an `IllegalArgumentException` naming the accepted set, not a silent fallback.

**Why a VALUE rather than a second flag.** A separate `--optimize-size` / `-Os` -- or, for the off switch, a `--no-optimize` / `-O0` -- sitting next to `--optimize` does not say how the two relate — a reader hitting it in a build script cannot tell whether it replaces `--optimize`, adds to it, or contradicts it. A value cannot be read that way. The CLI consequence is that `--optimize` stays in `CliOptions.noValueKeys` and the parser learned the `--key=value` form instead (`CliOptions.build`): moving it out of that set would make `rontolisp app.lisp --optimize -o out.wasm` read `-o` as the level.

### What `SIZE` declines, and what that costs

Only the wasm-GC backends (Preview 1 AND `--component`) have anything to trade: the two emissions that deliberately spend bytes on speed, both **on at `off` and `default` alike** — integer expression-tree fusion (`.kb/wasm-int-fusion.md`; a fused site emits its tree TWICE, raw plus the generic fallback) and unboxed dual-representation locals (`.kb/wasm-unboxed-locals.md`). One predicate switches both: `WasmIntFusionCompiler.speedTradesEnabled(ctx)`, read at the three fusion entry points and at `WasmLetCompiler`'s eligibility scan. `--no-gc` accepts the level and emits byte-identical output (pinned by `theSizeLevelIsADocumentedNoOpOnThisBackend` in `NoGcWasmCompilerTest`). The JVM backend accepted it as a documented no-op until `.todo/457` (2026-08-22) gave it its first speed-for-size trade, the typed numeric loops (`.kb/jvm-typed-loops.md`: a `dotimes` over packed float arrays emits its body up to three times -- two typed variants plus the boxed fallback), and `.todo/412` (2026-08-26) its second, integer expression-tree fusion plus unboxed dual-representation locals (`.kb/jvm-int-fusion.md`: every outlined `_fx$N` method carries its tree twice, raw plus the generic fallback). `Ctx.typedLoops` and `Ctx.intFusion`, both `!prefersSizeOverSpeed()`, switch them; a program WITHOUT any of those shapes is still byte-identical across the levels (`theSizeLevelChangesNothingWithoutASpeedForSizeTrade` in `JvmLispCompilerTest`) and one with them is smaller and slower at `=size` (`typedLoopsMatchTheBoxedPathAndTheSizeLevelDeclinesThem`, `fusedIntegerExpressionTreesMatchTheGenericPath`). Accepted everywhere rather than rejected so one build script can pass it for every target.
...
- **the names a runtime SYMBOL designator can resolve.** On WASM this source is live when the registry is (`usesEval || usesRuntimeDesignator || usesApplyRuntime` since todo-315 -- the apply tier keeps symbol designators resolving without the interpreter, `.kb/eval-runtime.md`). `_lookup` matches interned offsets (WASM) / string constants (JVM), so a registry row is reachable only when the program already put that exact name there for another reason — a quoted symbol, a string literal, an `intern` of a literal. The probe set is **`Ctx.spelledLiterals`** on both backends (todo-334, 2026-08-12): every spelling Pass 2 emits as a runtime VALUE, recorded in `Wasm/JvmEmitHelper.compileStringLiteral` exactly as `valueFuncIds` records closures — during emission, so a literal a macro synthesizes into a compiled body still counts. It used to be the whole string table / constant pool (`StringTable.isInterned` / `ConstantPool.hasStringConstant`), and that over-approximation had a body count: the instance-layout directory interns every class's slot names (`WasmInstanceLayouts.emit`), the printer prologue interns `"-"`/`"/"`, the JVM layout tables mint theirs — none of them producible as a designator at run time, and every one armed any same-named defun's row. Each name is tried seven ways: canonical, the `::`->`:` alias row's spelling, the bare member name after the last colon — and (2026-08-08) the FRAMED string-literal spelling of the full name and of the member (`"NAME"`, quotes included: a string literal interns via `LispString.literal()`, so `(intern "RUN" pkg)` — clack's handler discovery — spells `"RUN"`, not `RUN`; before this the probe missed every literal-string designator on both backends), plus the two package-less SYMBOL spellings whose name is the member, `:member` and `#:member` (`uiop:symbol-call :pkg :member` spells both halves as keywords; `(uiop:symbol-call '#:pkg '#:member ...)` — dexador's, and the spelling a `.asd` uses so that reading the form cannot require the package — spells them uninterned, and `(string '#:MEMBER)` is `"MEMBER"` exactly as `(string :MEMBER)` is. The uninterned one is todo-404: before it, that form compiled and then died undefined at run time).
  ...
  **The four widened spellings apply only while the program contains a symbol BUILDER
  ...
  a string, keyword or uninterned-symbol constant into a designator, so a gate-closed program stops paying
