...
and the `read-char` family under `(scheme base)`, whatever the tag in `SchemeBuiltins`
says; the keywords of `(scheme base)` (and `import`, which no library exports) are the
