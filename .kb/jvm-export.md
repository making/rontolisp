...
no directory, which the CLI's writer allows for), and so does a shipped template bridge
(`Name$JavaBridge.class`, `Name$GeomBridge.class`, `.kb/template-class-embedding.md`). A split keeps the exports and the defuns
