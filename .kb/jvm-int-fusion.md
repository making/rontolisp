...
- **The counted-loop STEP is emitted INLINE, ahead of the outlined method**
  (`emitRawStepFastPath`). `(+ i c)` / `(- i c)` / `(1+ i)` / `(1- i)` over a
  dual-representation local or promoted global, assigned straight back into
  one (both halves of the triple through the same field-aware emitters), is the shape
