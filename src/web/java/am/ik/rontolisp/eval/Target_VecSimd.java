 ...
 * {@link VecSimd#install(Environment)} are the only two methods that reference
 * {@code VecSimdKernels} (the sole holder of the Vector API), so substituting both makes
 * that class unreachable and keeps the incubator module out of the image. The playground
 * never sets the {@code --simd} flag anyway, so reporting "unavailable" is also the
 * truthful answer: the scalar {@code vec.lisp} kernels run instead.
 *
 * <p>
...
final class Target_VecSimd {
	...
	@Substitute
	static void install(Environment globalEnv) {
