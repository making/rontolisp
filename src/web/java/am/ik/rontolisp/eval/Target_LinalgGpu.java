 ...
 * {@link LinalgGpu#description()}, {@link LinalgGpu#install(Environment, LispEvaluator)}
 * and {@link LinalgGpu#installVec(Environment, LispEvaluator)} are the only four ENTRY
 * POINTS into {@code LinalgGpuKernels} (the holder of the single {@code am.ik.gpu}
 * reference), so substituting all four makes that class -- and with it both device
 * bindings, the CUDA one and the Metal one -- unreachable. Adding a new public method to
 * {@code LinalgGpu} that touches the kernels would break that, and only the Pages
 * workflow's Web Image build would catch it. The
