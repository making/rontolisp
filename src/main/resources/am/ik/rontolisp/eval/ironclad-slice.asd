...
;;; where it stands, so the classes it names must already be registered. A branch
;;; naming a class from a subsystem outside this slice (scrypt, argon2, bcrypt,
;;; hmac-kdf) compiles to a runtime error instead -- those KDFs are simply not
;;; available here.
;;;
;;; kdf/hmac.lisp (HKDF, RFC 5869) is deliberately absent: it builds its output with
;;; (apply #'concatenate '(vector (unsigned-byte 8)) ...) and concatenate supports
;;; only the string result type.
(defsystem "ironclad/kdfs"
  :depends-on ("ironclad/core" "ironclad/kdf/pkcs5")
  :components ((:module "src"
                :components ((:module "kdf"
                              :components ((:file "kdf")))))))
