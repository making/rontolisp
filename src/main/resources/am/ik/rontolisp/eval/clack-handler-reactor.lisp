;; clack.handler.reactor: the Clack handler backend that is a HOST-DRIVEN
;; REACTOR on EVERY backend -- run stores the application and returns, and the
;; host (a Cloudflare Worker's src/index.js, a browser page, a node or JVM
;; embedding, or a check script on the interpreter) calls DISPATCH with a JSON
;; request string and gets a JSON response string back. Satisfies the built-in
;; ASDF system "clack-handler-reactor" (and its dotted alias
;; "clack.handler.reactor", the spelling lack's find-package-or-load derives
;; from the package name). Like clack-handler-rontolisp the package is NOT
;; seeded in PackageRegistry, so this shim carries its own defpackage (the
;; leaf-module pattern).
;;
...
;; reactor host no longer NEEDS this designator -- one :rontolisp source covers
;; every host. What this backend still says is "host-driven EVERYWHERE": on the
;; interpreter and the JVM :rontolisp binds a real socket, while this backend
;; stores the app there too, which is what lets a Worker (or a browser, node or
;; JVM embedding) be developed and driven through `dispatch` on the
;; interpreter, where the edit/run loop costs nothing.
;;
;; The name is the TRANSPORT, not a deployment vendor: nothing below is
;; Cloudflare-specific, and the whole lineage under it (http-reactor.lisp,
;; rontolisp::%http-reactor-*, #+rontolisp-reactor) is already named for the
;; mechanism. A vendor-named backend would be one that surfaces something only
;; that vendor has -- a Worker's env/ctx bindings, say -- and it would layer on
;; top of this one.
;;
...
;; construction, no :thread-support), so a reactor source may omit it.

(defpackage :clack.handler.reactor
  (:use :cl)
