...
(defun tiny-routes.middleware.path-template::%lite-check-template (path-template)
...
(defun tiny-routes.middleware.path-template::%lite-parse-template (path-template)
  ...
  (let ((parts nil)
        (len (length path-template))
        (lit-start 0)
        (i 0))
    (do ()
        ((>= i len))
      (if (and (char= (char path-template i) #\:)
               (< (+ i 1) len)
          ...
          (let ((name-end (+ i 1)))
            (do ()
                ((or (>= name-end len)
                     (not (tiny-routes.middleware.path-template::%lite-token-char-p
                           (char path-template name-end)))))
              (setq name-end (+ name-end 1)))
            (when (> i lit-start)
              (setq parts (cons (subseq path-template lit-start i) parts)))
            (setq parts
                  (cons (intern (string-upcase (subseq path-template (+ i 1) name-end)) :keyword)
                        parts))
            (setq i name-end)
        ...
        (t
         (let ((limit pos)
           ...
           (do ((end limit (- end 1))
             ...
             (let ((rest (tiny-routes.middleware.path-template::%lite-match-parts
                          (cdr parts) path end)))
               (unless (eq rest :fail)
                 (setq found (cons (subseq path pos end) rest)))))))))

(defun tiny-routes.middleware.path-template::make-path-template-exact-matcher (path-template)
...
(defun tiny-routes.middleware.path-template::make-path-template-keyword-matcher (path-template)
...
groups when path matches PATH-TEMPLATE."
  ...
  (let ((parts (tiny-routes.middleware.path-template::%lite-parse-template path-template))
        (token-names nil))
    (dolist (part parts)
      (when (keywordp part)
        (setq token-names (cons part token-names))))
    ...
    (lambda (path)
      (let ((values (tiny-routes.middleware.path-template::%lite-match-parts parts path 0)))
        (if (eq values :fail)
            nil
            (let ((result nil))
              (do ((names token-names (cdr names))
                   (vals values (cdr vals)))
                  ((null names) (nreverse result))
                (setq result (cons (car vals) (cons (car names) result))))))))))

(defun tiny-routes.middleware.path-template::make-path-template-regex-matcher (path-template)
  (check-type path-template string)
  (error "tiny-routes/lite: :regex path templates need cl-ppcre -- load the full \"tiny-routes\" system instead of \"tiny-routes/lite\""))

(defun tiny-routes.middleware.path-template:wrap-request-path-info-matcher (handler path-info-matcher)
  ...
  (lambda (request)
    (let* ((path-info (tiny-routes.request:path-info request ""))
      ...
      (cond ((null params) nil)
            ((listp params)
             (funcall handler
                      (tiny-routes.request:request-append request :path-parameters params)))
            (t (funcall handler request))))))

(defun tiny-routes.middleware.path-template:wrap-request-matches-path-template (handler path-template &key regex)
...
expression."
  ...
  (cond ((or (null path-template)
         ...
         handler)
        (regex
         (tiny-routes.middleware.path-template:wrap-request-path-info-matcher
          handler
          (tiny-routes.middleware.path-template::make-path-template-regex-matcher path-template)))
        ((find #\: path-template)
         (tiny-routes.middleware.path-template:wrap-request-path-info-matcher
          handler
          (tiny-routes.middleware.path-template::make-path-template-keyword-matcher path-template)))
        (t
         (tiny-routes.middleware.path-template:wrap-request-path-info-matcher
          handler
          (tiny-routes.middleware.path-template::make-path-template-exact-matcher path-template)))))

(defun tiny-routes.middleware.path-template:path-parameter (request path-parameter &optional default)
...
(defmacro tiny-routes.middleware.path-template:with-path-parameters (vars path-parameters &body body)
