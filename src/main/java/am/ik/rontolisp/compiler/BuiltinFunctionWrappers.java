...
public final class BuiltinFunctionWrappers {
	...
	private static final List<WrapperDef> WRAPPER_DEFS = List.of(
			...
			binary(LispNames.FIND), binary(LispNames.POSITION), binary(LispNames.ASSOC), binary(LispNames.EVERY),
			binary(LispNames.SOME), binary(LispNames.REMOVE), binary(LispNames.REMOVE_IF),
			// Predicates (arity 1)
