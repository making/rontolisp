	...
	private static List<LispVal> forms() {
		...
		if (cached == null) {
			synchronized (StdinLibrary.class) {
				cached = forms;
				if (cached == null) {
					cached = LispReader.readAllFromString(readResource("stdin.lisp"));
					forms = cached;
				}
			}
		}
	...
	private static List<LispVal> stubForms() {
		...
		if (cached == null) {
			synchronized (StdinLibrary.class) {
				cached = stubForms;
				if (cached == null) {
					cached = LispReader.readAllFromString(readResource("stdin-stub.lisp"));
					stubForms = cached;
				}
			}
		}
	...
	private static List<LispVal> dispatchForms() {
		...
		if (cached == null) {
			synchronized (StdinLibrary.class) {
				...
				if (cached == null) {
					cached = LispReader.readAllFromString(readResource("stdin-dispatch.lisp"));
