...
public final class HostFetchLibrary {
	...
	// One parse per SHAPE, not one per JVM: the shapes generate different source, and
	// several appear in one JVM (the test suite compiles them side by side). Keyed by
	// boundary AND the reentrant bit, because the streaming source differs under
	// --reentrant (the envelope one does not, and shares its entry).
	private static final Map<String, List<LispVal>> FORMS = new java.util.HashMap<>();
	...
	public static List<LispVal> process(List<LispVal> program, HostBoundary boundary) {
		return process(program, boundary, false);
	}

	/**
	 * The {@code --reentrant}-aware overload of {@link #process(List, HostBoundary)}.
	 * @param program the top-level forms
	 * @param boundary which shape of the reply body was asked for
	 * @param reentrant whether the module overlaps calls on one instance
	 * ({@code --reentrant}): the streaming reply-body import then takes a leading
	 * {@code :int} REPLY ID -- minted by the host per fetch, carried back in the reply
	 * head's {@link FetchResponseShape#HOST_BODY_ID_KEY} -- so each body stream drains
	 * its own reply and the serialised boundary's one-cursor "superseded" counter does
	 * not exist. The envelope boundary carries no cursor and is unchanged by the flag.
	 * @return the program, with the lowering appended when it fetches
	 */
	public static List<LispVal> process(List<LispVal> program, HostBoundary boundary, boolean reentrant) {
		if (!referencesFetch(program)) {
			return program;
		}
		List<LispVal> out = new java.util.ArrayList<>(program);
		out.addAll(forms(boundary, reentrant));
		return out;
	}
	...
	public static List<LispVal> forms(HostBoundary boundary) {
		return forms(boundary, false);
	}

	/**
	 * The {@code --reentrant}-aware overload of {@link #forms(HostBoundary)}.
	 * @param boundary which shape of the reply body was asked for
	 * @param reentrant whether the module overlaps calls ({@code --reentrant})
	 * @return the lowering forms
	 */
	public static List<LispVal> forms(HostBoundary boundary, boolean reentrant) {
		// The envelope source ignores the flag, so both spellings share one entry --
		// keyed on the shape that is really generated, not on the arguments.
	...
	public static String source(HostBoundary boundary) {
		return source(boundary, false);
	}

	/**
	 * The {@code --reentrant}-aware overload of {@link #source(HostBoundary)}.
	 * @param boundary which shape of the reply body was asked for
	 * @param reentrant whether the module overlaps calls ({@code --reentrant})
	 * @return the Lisp source of the lowering
	 */
	public static String source(HostBoundary boundary, boolean reentrant) {
		boolean withId = boundary.bodiesOutOfBand() && reentrant;
		...
		src.append(withId ? """
				  ...
				  ;; pulling it is what the stream below does, naming the reply id the
				  ;; head carried back. Each reply has its OWN host-side cursor, so
				  ;; nothing is superseded and no open-reply counter exists.
				""" : boundary.bodiesOutOfBand() ? """
				  ;; What it settles to is the HEAD -- the body is still on the wire, and
	...
	private static void appendBody(StringBuilder src, HostBoundary boundary, boolean withId) {
	...
	// The --reentrant spelling of the same import: a leading :int REPLY ID, minted by
	// the host per fetch and carried back in the reply head's "body-id" key
	// (FetchResponseShape.HOST_BODY_ID_KEY). With every pull naming its reply, the host
	// keeps one cursor per reply instead of one cursor -- so overlapped calls (and a
	// second fetch inside one call) each drain their own, nothing is superseded, and the
	// serialised shape's open-reply counter does not exist here.
	private static void appendBodyImportWithId(StringBuilder src) {
		src.append("(rontolisp:wasm-import 'rontolisp::%host-fetch-read-body :from \"")
			.append(IMPORT_MODULE)
			.append("\" :as \"")
			.append(BODY_IMPORT_FIELD)
			.append("\"\n                       :params '(:int) :returns :bytes :async t)\n");
		src.append("""
				(defun rontolisp::%host-fetch-pull (id)
				  ;; One chunk of the reply ID names, through the caller-passes-the-buffer
				  ;; import: the module owns the buffer and the host writes into it.
				  (let* ((buf (rontolisp::%http-reactor-buffer\s""").append(HttpReactorInliner.CHUNK_BYTES).append("""
				))
				         (n
				          (rontolisp::%http-reactor-force
				           (rontolisp::%host-fetch-read-body id buf))))
				    ;; A NEGATIVE count is the host reporting that the transfer failed
				    ;; after the head crossed -- the mid-body failure every other backend
				    ;; signals at the drain. 0 is end of stream, and an id the host no
				    ;; longer holds a reply for is end of stream too.
				    (when (and n (< n 0))
				      (error "fetch: the response body failed mid-transfer"))
				    (rontolisp::%http-reactor-chunk buf n)))
				(defun rontolisp::%host-fetch-body (in-band id)
				  ;; The :body of the result plist. The in-band "body" key still wins when
				  ;; a host fills it (normalized first, so JSON null is "not here"); a head
				  ;; that carried neither the key nor a reply id is a bodiless reply, the
				  ;; empty stream.
				  (rontolisp::%http-reactor-body-stream
				   (or (rontolisp::%http-reactor-source in-band)
				       (if id (lambda () (rontolisp::%host-fetch-pull id)) nil))))
				""");
	}
