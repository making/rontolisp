...
final class WasmEvalRuntimeBuilder {
	...
	static byte[] buildEvalBody(SpecialFormOffsets off) {
		...
		// --- 2. integers (i31), floats and closures are self-evaluating ---
		for (int heapType : new int[] { Type.I31.code(), WasmLispCompiler.TYPE_FLOAT, WasmLispCompiler.TYPE_CLOSURE }) {
			getLocal(w, VAL);
			refTest(w, heapType);
			w.write(Instruction.IF, 0x40);
			getLocal(w, VAL);
