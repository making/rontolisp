...
/**
 * JLine-based interactive REPL. This class is isolated from {@link FemtoLispCli} so that
	...
	static void run(LispEvaluator evaluator, PrintStream out, StringBuilder buffer) {
		try (Terminal terminal = TerminalBuilder.builder().system(true).build()) {
			LineReader lineReader = LineReaderBuilder.builder()
				.terminal(terminal)
				.variable(LineReader.HISTORY_FILE, Path.of(System.getProperty("user.home"), ".femtolisp_history"))
				.build();
			while (true) {
				...
				if (FemtoLispCli.isBalanced(buffer.toString())) {
					FemtoLispCli.evalBuffer(evaluator, out, buffer);
				}
			}
		}
