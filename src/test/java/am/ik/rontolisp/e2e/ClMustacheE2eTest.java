...
class ClMustacheE2eTest extends AsdfLibraryE2eSupport {
	...
	private static final String EXERCISE = """
			...
			(terpri)
			(with-open-file (s "target/mustache-e2e-%%WORK%%.mustache" :direction :output :if-exists :supersede)
			  (write-string "file says {{who}}" s))
			(princ (with-output-to-string (out)
			         (funcall (mustache:compile-template (pathname "target/mustache-e2e-%%WORK%%.mustache")) '((:who . "hi")) out)))
			(terpri)
